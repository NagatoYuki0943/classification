...
class InceptionAux(nn.Module):
    ...
    辅助输出的形状和最终输出的形状完全相同,形状都是[batch, num_classes];只在训练时会有,验证时没有
    '''
    def __init__(self, in_channels,   # 输入通道个数
                        num_classes): # 最后分类数
        super().__init__()
        self.averagePool = nn.AvgPool2d(kernel_size=5, stride=3)  # 池化  [N, C, 14, 14] => [N, C, 4, 4]
        self.conv = BasicConv2d(in_channels, 128, kernel_size=1)  # 卷积  output[N, 128, 4, 4]
...
class GoogLeNet(nn.Module):
    def __init__(self, num_classes=1000,    # 分类个数
        ...
        self.maxpool1 = nn.MaxPool2d(3, stride=2, ceil_mode=True)   # 参数ceil_mode: 如果下采样最右边和下边的不足一组,不会丢弃,而是继续使用
