...
class ChannelAttention(nn.Module):
    def __init__(self, channel, squeeze_channels):
        super().__init__()
        ...
        self.fc = nn.Sequential(
            nn.Conv2d(channel, squeeze_channels, 1, bias=False),
            nn.ReLU(),
            nn.Conv2d(squeeze_channels, channel, 1, bias=False),
        )
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # b, 2, h, w -> b, 1, h, w
        x = self.conv(x)
        return self.sigmoid(x)  # 这里也有sigmoid
...
class cbam_block(nn.Module):
    def __init__(self, channel, squeeze_channels, kernel_size=7):
        super().__init__()
        self.channelattention = ChannelAttention(channel, squeeze_channels)
...
class MBConv(nn.Module):
    ...
    def __init__(
        ...
        # 更换为eca
        layers.append(cbam_block(expanded_channels, squeeze_channels))
...
def _efficientnet_model(
    ...
    pretrained: bool,
    progress: bool,
    num_classes=1000,  # 这里显示指定,防止被传进EfficientNet中导致无法载入权重
    **kwargs: Any,
) -> EfficientNet:
    ...
    if pretrained:
        ...
        # 打印missing_keys和unexpected_keys
