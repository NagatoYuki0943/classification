...
class ConvNormActivation(nn.Sequential):
    ...
    def __init__(
        ...
        if activation_layer is not None:
            inplace = None  # 添加这一行,GELU没有inplace参数
...
class MBConv(nn.Module):
    ...
    def __init__(self,
        ...
        # 激活函数
        activation_layer = nn.GELU  # 更换为GELU
        ...
        layers.append(se_layer(expanded_channels, squeeze_channels, activation=partial(nn.GELU)))       # 更换为GELU
...
class EfficientNet(nn.Module):
    def __init__(
        ...
        layers.append(ConvNormActivation(3, firstconv_output_channels, kernel_size=3, stride=2, norm_layer=norm_layer, activation_layer=nn.GELU))   # 更换为GELU
        ...
        layers.append(ConvNormActivation(lastconv_input_channels, lastconv_output_channels, kernel_size=1, norm_layer=norm_layer, activation_layer=nn.GELU))    # 更换为GELU
...
if __name__ == "__main__":
    ...
    model = efficientnet_b0(pretrained=False, num_classes=10).to(device)
