...
class MBConv(nn.Module):
    ...
    def __init__(self,
        ...
        # 激活函数
        activation_layer = nn.Mish  # 更换为Mish
        ...
        layers.append(se_layer(expanded_channels, squeeze_channels, activation=partial(nn.Mish, inplace=True)))     # 更换为MIsh
...
class EfficientNet(nn.Module):
    def __init__(
        ...
        layers.append(ConvNormActivation(3, firstconv_output_channels, kernel_size=3, stride=2, norm_layer=norm_layer, activation_layer=nn.Mish))   # 更换为Mish
        ...
        layers.append(ConvNormActivation(lastconv_input_channels, lastconv_output_channels, kernel_size=1, norm_layer=norm_layer, activation_layer=nn.Mish))    # 更换为Mish
