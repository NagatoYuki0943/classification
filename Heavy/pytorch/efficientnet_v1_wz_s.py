        ...
        layers.update(
            {
                "dwconv": ConvBNActivation(
                    cnf.expanded_c,  # in_channel == out_channel
                    cnf.expanded_c,
    ...
    def __init__(
        ...
        for stage, args in enumerate(default_cnf):
            ...
            for i in range(round_repeats(cnf.pop(-1))):
                ...
                index = (
                    str(stage + 1) + chr(i + 97)
                )  # 1a, 2a, 2b, ... 记录顺序,通过这个方法能记录当前MBConv结构是属于第几个stage中的第几个MBConv结构
