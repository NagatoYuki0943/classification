...
#     ConvBNAct(k=2, s=1)     │
#           │                 │
#    pad(0, 1, 0, 1)      MaxPool2d(k=2, s=1, ceil_mode=True)
#           │                 │
#     ConvBNAct(k=2, s=1)     │
#           │                 │
#           └────── cat ──────┘
...
#   │      │                 │         │       conv
#   │      │                 │         │        ├ ─ ─ ─ ─ ┐
#   │      │                 │         │        │        conv  总共有layer_num个conv
#   │      └──── concat ─────┴─────────┴────────┘─ ─ ─ ─ ─┘
#   │               │
#   │          aggregation(v1: ConvBNAct + EseModule; v2: ConvBNAct + ConvBNAct)
#   │               │
#   │           drop_path
#   │               │
#   └────────────── +
...
class HighPerfGpuBlock(nn.Module):
    ...
    def forward(self, x):
        identity = x
        # 级联多个conv,每个conv的输出都被保存并concat到一起
        output = [x]
        for layer in self.layers:
            x = layer(x)
            output.append(x)
        x = torch.cat(output, dim=1)
        # 对concat结果进行feature aggregation,调整通道数
...
class HighPerfGpuStage(nn.Module):
    def __init__(
        ...
        # 重复堆叠HighPerfGpuBlock
        blocks_list = []
        for i in range(block_num):
            blocks_list.append(
                HighPerfGpuBlock(
                    in_chs if i == 0 else out_chs,  # 第一次会改变通道
                    mid_chs,
                    out_chs,
                    layer_num,
