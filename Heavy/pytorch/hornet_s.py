...
class gnconv(nn.Module):
    ...
    def forward(self, x, mask=None, dummy=False):
        ...
        pwa, abc = torch.split(fused_x, (self.dims[0], sum(self.dims)), dim=1)  # [B, 128, 56, 56] -> [B, 32, 56, 56], [B, 96, 56, 56]

        dw_abc = self.dwconv(abc) * self.scale                                  # [B, 96, 56, 56] -> [B, 96, 56, 56]

        dw_list = torch.split(dw_abc, self.dims, dim=1)                         # [B, 96, 56, 56] -> [B, 32, 56, 56] -> [B, 64, 56, 56]     这里可能拆分为多个,例子是第一次调用
        x = pwa * dw_list[0]                                                    # [B, 32, 56, 56] * [B, 32, 56, 56] = [B, 32, 56, 56]

        for i in range(self.order -1):
            x = self.pws[i](x) * dw_list[i+1]   # [B, 32, 56, 56] -> [B, 64, 56, 56] * [B, 64, 56, 56] = [B, 64, 56, 56]
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.gamma1 is not None:
            ...
            gamma1 = 1
        x = x + self.drop_path(gamma1 * self.gnconv(self.norm1(x))) # [B, C, H, W] -> [B, C, H, W]
        # mlp
        input = x
...
class HorNet(nn.Module):
    ...
    def forward_features(self, x):
        for i in range(4):  # 4个stage
            x = self.downsample_layers[i](x)    # stem包含进去  [B, 3, 224, 224] -> [B, 64, 56, 56] -> [B, 128, 28, 28] -> [B, 256, 14, 14] -> [B, 512, 7, 7]
