...
class GlobalLocalFilter(nn.Module):
    def __init__(self, dim, h=14, w=8):
        super().__init__()
        self.dw = nn.Conv2d(
            dim // 2, dim // 2, kernel_size=3, padding=1, bias=False, groups=dim // 2
        )
...
class gnconv(nn.Module):
    ...
    def forward(self, x, mask=None, dummy=False):
        B, C, H, W = x.shape  # [B, 64, 56, 56]

        fused_x = self.proj_in(x)  # [B, 64, 56, 56] -> [B, 128, 56, 56]
        pwa, abc = torch.split(
            fused_x, (self.dims[0], sum(self.dims)), dim=1
        )  # [B, 128, 56, 56] -> [B, 32, 56, 56], [B, 96, 56, 56]

        dw_abc = self.dwconv(abc) * self.scale  # [B, 96, 56, 56] -> [B, 96, 56, 56]

        dw_list = torch.split(
            dw_abc, self.dims, dim=1
        )  # [B, 96, 56, 56] -> [B, 32, 56, 56] -> [B, 64, 56, 56]     这里可能拆分为多个,例子是第一次调用
        x = pwa * dw_list[0]  # [B, 32, 56, 56] * [B, 32, 56, 56] = [B, 32, 56, 56]

        for i in range(self.order - 1):
            x = (
                self.pws[i](x) * dw_list[i + 1]
            )  # [B, 32, 56, 56] -> [B, 64, 56, 56] * [B, 64, 56, 56] = [B, 64, 56, 56]

        x = self.proj_out(x)  # [B, 64, 56, 56] -> [B, 64, 56, 56]
...
class Block(nn.Module):
    ...
    def __init__(self, dim, drop_path=0.0, layer_scale_init_value=1e-6, gnconv=gnconv):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.gamma1 is not None:
            gamma1 = self.gamma1.view(C, 1, 1)  # [C] -> [C, 1, 1]
        else:
            gamma1 = 1
        x = x + self.drop_path(
            gamma1 * self.gnconv(self.norm1(x))
        )  # [B, C, H, W] -> [B, C, H, W]
        # mlp
        input = x
        x = x.permute(0, 2, 3, 1)  # [B, C, H, W] -> [B, H, W, C]
        x = self.norm2(x)
        x = self.pwconv1(x)  # [B, H, W, C] -> [B, H, W, n*C]
        x = self.act(x)
        x = self.pwconv2(x)  # [B, H, W, n*C] -> [B, H, W, C]
        if self.gamma2 is not None:
            x = self.gamma2 * x
        x = x.permute(0, 3, 1, 2)  # [B, H, W, C] -> [B, C, H, W]

        x = input + self.drop_path(x)  # [B, H, W, C] + [B, H, W, C] = [B, H, W, C]
        return x
...
class HorNet(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.downsample_layers.append(stem)  # stem包含进下采样
        for i in range(3):
    ...
    def forward_features(self, x):
        for i in range(4):  # 4个stage
            x = self.downsample_layers[
                ...
                x
            )  # stem包含进去  [B, 3, 224, 224] -> [B, 64, 56, 56] -> [B, 128, 28, 28] -> [B, 256, 14, 14] -> [B, 512, 7, 7]
            for j, blk in enumerate(self.stages[i]):
                x = blk(x)  # 形状不变
        return self.norm(x.mean([-2, -1]))  # [B, 512, 7, 7] -> [B, 512]

    def forward(self, x):
        x = self.forward_features(x)  # [B, 3, 224, 224] -> [B, 512]
...
if __name__ == "__main__":
    ...
    model = hornet_tiny_7x7(pretrained=False, num_classes=5).to(device)
    ...
    if False:
        onnx_path = "hornet_tiny_7x7.onnx"
        torch.onnx.export(
            model,
            x,
            onnx_path,
