...
# 返回act创建器,在NormFreeBlock中使用 返回值() 得到GammaAct
...
# ---------------------------------------------------------------#
# 1x1Conv减小通道数,2次3x3GConv通道不变,1x1Conv还原通道数
#
#   transition block                    non-transition block
#          in                                  in
#           │                                   ├───────────┐
#           │                                   │           │
#         1/beta                              1/beta        │
#           │                                   │           │
#       Scaled Act                          Scaled Act      │
#           │                                   │           │
#           ├───────────────┐                   │           │
#           │               │                   │           │
#       1x1 WS-Conv     Avg Pool           1x1 WS-Conv      │
#           │               │                   │           │
#       Scaled Act     1x1 WS-Conv          Scaled Act      │
#           |               │                   |           │
#    3x3 WS s2,C/G=128      │            3x3 WS s2,C/G=128  │   C/G=128 128channel一组
#           │               │                   │           │
#       Scaled Act          │               Scaled Act      │
#           |               |                   |           |
#    3x3 WS s2,C/G=128      │            3x3 WS s2,C/G=128  │
#           │               │                   │           │
#       Scaled Act          │               Scaled Act      │
#           │               │                   │           │
...
class NormFreeBlock(nn.Module):
    ...
    def __init__(
        ...
        if group_size and group_size % ch_div == 0:
            mid_chs = (
                group_size * groups
            )  # correct mid_chs if group_size divisible by ch_div, otherwise error
        self.alpha = alpha
        self.beta = beta
        ...
        self.conv2 = conv_layer(
            mid_chs, mid_chs, 3, stride=stride, dilation=first_dilation, groups=groups
        )
        if extra_conv:
            self.act2b = act_layer(inplace=True)
            self.conv2b = conv_layer(
                mid_chs, mid_chs, 3, stride=1, dilation=dilation, groups=groups
            )
        else:
            self.act2b = None
            self.conv2b = None
    ...
    def forward(self, x):  # 第一个stage不下采样
        # Scaled Act + beta                       stage1 first s=1                      stage2 first s=2                      stage2 second s=1
        out = (
            self.act1(x) * self.beta
        )  # [B, 128, 48, 48] -> [B, 128, 48, 48]  [B, 256, 48, 48] -> [B, 256, 48, 48]  [B, 512, 24, 24] -> [B, 512, 24, 24]

        # shortcut branch
        # 开始的shortcut是x,如果有下采样的话对out处理，所以short在ScaledAct之后
        shortcut = x
        if self.downsample is not None:
            shortcut = self.downsample(
                out
            )  # [B, 128, 48, 48] -> [B, 256, 48, 48]  [B, 256, 48, 48] -> [B, 512, 24, 24]  None

        # residual branch
        out = self.conv1(
            out
        )  # [B, 128, 48, 48] -> [B, 128, 48, 48]  [B, 256, 48, 48] -> [B, 256, 48, 48]  [B, 512, 24, 24] -> [B, 256, 24, 24]
        out = self.conv2(self.act2(out))
        # Scaled Act + 3x3DWConv s = 1 or 2
        if self.conv2b is not None:
            out = self.conv2b(
                self.act2b(out)
            )  # [B, 128, 48, 48] -> [B, 128, 48, 48]  [B, 256, 48, 48] -> [B, 256, 24, 24]  [B, 256, 24, 24] -> [B, 256, 24, 24]
        if self.attn is not None:
            out = self.attn_gain * self.attn(out)
        # Scaled Act + 1x1Conv
        out = self.conv3(
            self.act3(out)
        )  # [B, 128, 48, 48] -> [B, 256, 48, 48]  [B, 256, 24, 24] -> [B, 512, 24, 24]  [B, 256, 24, 24] -> [B, 512, 24, 24]
        if self.attn_last is not None:
            out = self.attn_gain * self.attn_last(out)  # same
...
class NormFreeNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            cfg.act_layer in _nonlin_gamma
        ...
        if cfg.gamma_in_act:
            act_layer = act_with_gamma(
                cfg.act_layer, gamma=_nonlin_gamma[cfg.act_layer]
            )
            conv_layer = partial(conv_layer, eps=cfg.std_conv_eps)
        else:
            act_layer = get_act_layer(cfg.act_layer)
            conv_layer = partial(
                conv_layer, gamma=_nonlin_gamma[cfg.act_layer], eps=cfg.std_conv_eps
            )
        attn_layer = (
            partial(get_attn(cfg.attn_layer), **cfg.attn_kwargs)
            if cfg.attn_layer
            else None
        )
        ...
        # --------------------------------------------------------------------------------------------------#
        #   开始的stem部分,下采样2次
        #   [B, 3, 192, 192] -> [B, 16, 96, 96] -> [B, 32, 96, 96] -> [B, 64, 96, 96] -> [B, 128, 48, 48]
        # --------------------------------------------------------------------------------------------------#
        ...
        # -------------------------------------------------------------#
        #   主干stages部分,4个stage,下采样3次，第一次只变化维度,不下采样
        # -------------------------------------------------------------#
        ...
        dilation = 1
        expected_var = 1.0
        stages = []  # 存放每个stage的Sequential,一共4个Sequential
        # 4个stage
        for stage_idx, stage_depth in enumerate(cfg.depths):
            stride = (
                1 if stage_idx == 0 and stem_stride > 2 else 2
            )  # 第一个stage的开始s=1,其余stage的开始stage=2
            ...
            for block_idx in range(cfg.depths[stage_idx]):
                ...
                blocks += [
                    NormFreeBlock(
                        ...
                        stride=stride
                        if block_idx == 0
                        else 1,  # 第一次的stage按照外部变量,后面的全为1
                        dilation=dilation,
                        first_dilation=first_dilation,
                        group_size=cfg.group_size,
                        bottle_ratio=1.0
                        if cfg.reg and first_block
                        else cfg.bottle_ratio,
                        ch_div=cfg.ch_div,
                        reg=cfg.reg,
                ...
                if block_idx == 0:
                    expected_var = (
                        1.0  # expected var is reset after first block of each stage
                    )
                expected_var += (
                    cfg.alpha**2
        ...
        if cfg.num_features:
            ...
            self.feature_info[-1] = dict(
                num_chs=self.num_features, reduction=net_stride, module=f"final_conv"
            )
        else:
    ...
    def forward_features(self, x):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            x = self.stages(
                x
            )  # [B, 128, 48, 48] -> [B, 256, 48, 48] -> [B, 512, 24, 24] -> [B, 1536, 12, 12] -> [B, 1536, 6, 6]
        x = self.final_conv(x)  # [B, 1536, 6, 6] -> [B, 3072, 6, 6]
        x = self.final_act(x)
        return x

    def forward_head(self, x, pre_logits: bool = False):
        # [B, 3072, 6, 6] -> [B, 3072, 1, 1] -> [B, 3072] -> [B, num_classes]
        return self.head(x, pre_logits=pre_logits) if pre_logits else self.head(x)

    def forward(self, x):
        x = self.forward_features(x)  # [B, 3, 192, 192] -> [B, 3072, 6, 6]
        x = self.forward_head(x)  # [B, 3072, 6, 6] -> [B, num_classes]
        return x
...
model_cfgs = dict(
    ...
    nf_seresnet101=_nfres_cfg(
        depths=(3, 4, 23, 3), attn_layer="se", attn_kwargs=dict(rd_ratio=1 / 16)
    ),
    nf_ecaresnet26=_nfres_cfg(
        depths=(2, 2, 2, 2), attn_layer="eca", attn_kwargs=dict()
    ),
    nf_ecaresnet50=_nfres_cfg(
        depths=(3, 4, 6, 3), attn_layer="eca", attn_kwargs=dict()
    ),
    nf_ecaresnet101=_nfres_cfg(
        depths=(3, 4, 23, 3), attn_layer="eca", attn_kwargs=dict()
    ),
)
...
if __name__ == "__main__":
    ...
    model = dm_nfnet_f0(pretrained=False, num_classes=5).to(device)
