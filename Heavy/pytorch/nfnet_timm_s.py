...
class NormFreeBlock(nn.Module):
    ...
    def forward(self, x):                       # 第一个stage不下采样
        ...
        out = self.act1(x) * self.beta          # [B, 128, 48, 48] -> [B, 128, 48, 48]  [B, 256, 48, 48] -> [B, 256, 48, 48]  [B, 512, 24, 24] -> [B, 512, 24, 24]

        # shortcut branch
        # 开始的shortcut是x,如果有下采样的话对out处理，所以short在ScaledAct之后
        shortcut = x
        if self.downsample is not None:
            shortcut = self.downsample(out)     # [B, 128, 48, 48] -> [B, 256, 48, 48]  [B, 256, 48, 48] -> [B, 512, 24, 24]  None

        # residual branch
        out = self.conv1(out)                   # [B, 128, 48, 48] -> [B, 128, 48, 48]  [B, 256, 48, 48] -> [B, 256, 48, 48]  [B, 512, 24, 24] -> [B, 256, 24, 24]
        out = self.conv2(self.act2(out))
        # Scaled Act + 3x3DWConv s = 1 or 2
        if self.conv2b is not None:
            out = self.conv2b(self.act2b(out))  # [B, 128, 48, 48] -> [B, 128, 48, 48]  [B, 256, 48, 48] -> [B, 256, 24, 24]  [B, 256, 24, 24] -> [B, 256, 24, 24]
        if self.attn is not None:
            out = self.attn_gain * self.attn(out)
        # Scaled Act + 1x1Conv
        out = self.conv3(self.act3(out))        # [B, 128, 48, 48] -> [B, 256, 48, 48]  [B, 256, 24, 24] -> [B, 512, 24, 24]  [B, 256, 24, 24] -> [B, 512, 24, 24]
...
class NormFreeNet(nn.Module):
    ...
    def __init__(
        ...
        for stage_idx, stage_depth in enumerate(cfg.depths):
            stride = 1 if stage_idx == 0 and stem_stride > 2 else 2 # 第一个stage的开始s=1,其余stage的开始stage=2
            ...
            for block_idx in range(cfg.depths[stage_idx]):
                ...
                blocks += [NormFreeBlock(
                    ...
                    stride=stride if block_idx == 0 else 1, # 第一次的stage按照外部变量,后面的全为1
                    dilation=dilation,
                    first_dilation=first_dilation,
    ...
    def forward_features(self, x):
        x = self.stem(x)        # [B, 3, 192, 192] -> [B, 16, 96, 96] -> [B, 32, 96, 96] -> [B, 64, 96, 96] -> [B, 128, 48, 48]
        if self.grad_checkpointing and not torch.jit.is_scripting():
            x = checkpoint_seq(self.stages, x)
        else:
            x = self.stages(x)  # [B, 128, 48, 48] -> [B, 256, 48, 48] -> [B, 512, 24, 24] -> [B, 1536, 12, 12] -> [B, 1536, 6, 6]
