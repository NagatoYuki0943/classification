    ...
    def __init__(
        ...
        for i in range(depth):
            block = block_constructor(
                width_in if i == 0 else width_out,  # 第一个完成 width_in => width_out 其余的维度不变,全是 width_out => width_out
                width_out,
                stride if i == 0 else 1,            # 第一个ResBottleneckBlock的步长为2,宽高减半,其余为1
                norm_layer,
                activation_layer,
                group_width,
...
class BlockParams:
    ...
    def __init__(
        ...
        bottleneck_multipliers: List[float],    # ResBottleneckBlock第一个1x1Conv维度变化倍率,默认为1,代表输出维度不变化,这样效果最好
        strides: List[int],
        se_ratio: Optional[float] = None,       # SE倍率
    ) -> None:
        self.depths = depths
