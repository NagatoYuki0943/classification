...
class RadixSoftmax(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.radix > 1:
            x = x.view(batch, self.cardinality, self.radix, -1).transpose(
                1, 2
            )  # [B, 128, 1, 1] -> [B, 1, 2, 64] -> [B, 2, 1, 64]
            x = F.softmax(x, dim=1)
            x = x.reshape(batch, -1)
        else:
            x = torch.sigmoid(x)
...
class SplitAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.radix > 1:
            x = x.reshape(
                (B, self.radix, RC // self.radix, H, W)
            )  # [B, 128, 56, 56] -> [B, 2, 64, 56, 56]
        ...
        x_attn = self.rsoftmax(x_attn).view(
            B, -1, 1, 1
        )  # [B, 128, 1, 1] -> [B, 2, 1, 64] 2代表2个radix
        if self.radix > 1:
            out = (x * x_attn.reshape((B, self.radix, RC // self.radix, 1, 1))).sum(
                dim=1
            )  # [B, 2, 64, 56, 56] * ([B, 2, 1, 64] -> [B, 2, 64, 1, 1]) = [B, 2, 64, 56, 56] -> [B, 64, 56, 56]
        else:
            out = x * x_attn
...
def resnest14d(pretrained=False, **kwargs) -> ResNet:
    ...
    return _create_resnest(
        "resnest14d", pretrained=pretrained, **dict(model_kwargs, **kwargs)
    )
...
def resnest26d(pretrained=False, **kwargs) -> ResNet:
    ...
    return _create_resnest(
        "resnest26d", pretrained=pretrained, **dict(model_kwargs, **kwargs)
    )
...
def resnest50d(pretrained=False, **kwargs) -> ResNet:
    ...
    return _create_resnest(
        "resnest50d", pretrained=pretrained, **dict(model_kwargs, **kwargs)
    )
...
def resnest101e(pretrained=False, **kwargs) -> ResNet:
    ...
    return _create_resnest(
        "resnest101e", pretrained=pretrained, **dict(model_kwargs, **kwargs)
    )
...
def resnest200e(pretrained=False, **kwargs) -> ResNet:
    ...
    return _create_resnest(
        "resnest200e", pretrained=pretrained, **dict(model_kwargs, **kwargs)
    )
...
def resnest269e(pretrained=False, **kwargs) -> ResNet:
    ...
    return _create_resnest(
        "resnest269e", pretrained=pretrained, **dict(model_kwargs, **kwargs)
    )
...
def resnest50d_4s2x40d(pretrained=False, **kwargs) -> ResNet:
    ...
    return _create_resnest(
        "resnest50d_4s2x40d", pretrained=pretrained, **dict(model_kwargs, **kwargs)
    )
...
def resnest50d_1s4x24d(pretrained=False, **kwargs) -> ResNet:
    ...
    return _create_resnest(
        "resnest50d_1s4x24d", pretrained=pretrained, **dict(model_kwargs, **kwargs)
    )
