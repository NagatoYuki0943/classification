...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channel, channel, stride=1, downsample=None, groups=1, width_per_group=64):
        ...
        width = int(channel * (width_per_group / 64.)) * groups
        ...
        # 1x1Conv调整通道
        self.conv3 = nn.Conv2d(width, channel*self.expansion, kernel_size=1, stride=1, bias=False)  # unsqueeze channels
...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        #                                   channel 重复次数
        self.layer1 = self._make_layer(block, 64, blocks_num[0])            # [b, 64, 56,56] -> [b,256, 56,56]
        self.layer2 = self._make_layer(block, 128, blocks_num[1], stride=2) # [b, 256,56,56] -> [b,512, 28,28]
        self.layer3 = self._make_layer(block, 256, blocks_num[2], stride=2) # [b, 512,28,28] -> [b,1024,14,14]
        self.layer4 = self._make_layer(block, 512, blocks_num[3], stride=2) # [b,1024,14,14] -> [b,2048, 7, 7]

        if self.include_top:
            self.avgpool = nn.AdaptiveAvgPool2d((1, 1))                     # [b,2048,7,7] -> [b,2048]
            self.fc = nn.Linear(512 * block.expansion, num_classes)         # [b,2048] -> [b,num_classes]
...
def resnet(version, block, pretrained=False, blocks_num=[2, 2, 2, 2], num_classes=1000, include_top=True, groups=1, width_per_group=64):
    model = ResNet(block, blocks_num, num_classes=num_classes, include_top=include_top, groups=groups, width_per_group=width_per_group)
    if pretrained:
...
def resnet18(pretrained=False, num_classes=1000, include_top=True):
    return resnet('resnet18', BasicBlock, pretrained, [2, 2, 2, 2], num_classes, include_top)


def resnet34(pretrained=False, num_classes=1000, include_top=True):
    return resnet('resnet34', BasicBlock, pretrained, [3, 4, 6, 3], num_classes, include_top)


def resnet50(pretrained=False, num_classes=1000, include_top=True):
    return resnet('resnet50', Bottleneck, pretrained, [3, 4, 6, 3], num_classes, include_top)


def resnet101(pretrained=False, num_classes=1000, include_top=True):
    return resnet('resnet101', Bottleneck, pretrained, [3, 4, 23, 3], num_classes, include_top)


def resnet152(pretrained=False, num_classes=1000, include_top=True):
    return resnet('resnet152', Bottleneck, pretrained, [3, 8, 36, 3], num_classes, include_top)
