...
def AlexNet_v1(im_height=224, im_width=224, num_classes=1000):
    ...
    input_image = layers.Input(
        shape=(im_height, im_width, 3), dtype="float32"
    )  # output(N, 224, 224, 3)
    ...
    x = layers.Conv2D(48, kernel_size=11, strides=4, activation="relu")(
        x
    )  # output(N, 55, 55, 48)
    #                    高宽减少到不要一半
    x = layers.MaxPool2D(pool_size=3, strides=2)(x)  # output(N, 27, 27, 48)
    x = layers.Conv2D(128, kernel_size=5, padding="same", activation="relu")(
        x
    )  # output(N, 27, 27, 128)
    x = layers.MaxPool2D(pool_size=3, strides=2)(x)  # output(N, 13, 13, 128)
    x = layers.Conv2D(192, kernel_size=3, padding="same", activation="relu")(
        x
    )  # output(N, 13, 13, 192)
    x = layers.Conv2D(192, kernel_size=3, padding="same", activation="relu")(
        x
    )  # output(N, 13, 13, 192)
    x = layers.Conv2D(128, kernel_size=3, padding="same", activation="relu")(
        x
    )  # output(N, 13, 13, 128)
    ...
    def __init__(self, num_classes=1000):
        ...
        self.features = Sequential(
            [
                ...
                layers.Conv2D(
                    48, kernel_size=11, strides=4, activation="relu"
                ),  # output(N, 55, 55, 48)
