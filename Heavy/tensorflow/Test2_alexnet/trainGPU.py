...
3:
    tf.data.Dataset.from_tensor_slices可以使用多线程 keras的ImageDataGenerator不会使用多线程
    所以这里使用手动获取数据的方式,类似于pytorch的Dataset
...
def main():
    ...
    data_class = [
        cla
        for cla in os.listdir(train_dir)
    ...
    # 训练集
    # tf.data.Dataset.from_tensor_slices可以使用多线程 keras不会使用多线程
    train_dataset = tf.data.Dataset.from_tensor_slices(
        (train_image_list, train_label_list)
    )
    train_dataset = (
        train_dataset.shuffle(buffer_size=train_num)
    ...
    val_dataset = (
        val_dataset.map(process_path, num_parallel_calls=tf.data.experimental.AUTOTUNE)
        .repeat()
        .batch(batch_size)
    )
    ...
    def train_step(images, labels):
        with tf.GradientTape() as tape:
            predictions = model(images, training=True)  # 训练: training=True
    ...
    def test_step(images, labels):
        predictions = model(images, training=False)  # 测试: training=False
    ...
    for epoch in range(1, epochs + 1):
        ...
        if test_loss.result() < best_test_loss:
            model.save_weights(
                "./save_weights/myAlex.ckpt".format(epoch), save_format="tf"
            )
