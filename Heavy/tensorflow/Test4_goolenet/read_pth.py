...
def rename_var(pth_path, new_ckpt_path, num_classes):
    ...
    with tf.Graph().as_default(), tf.compat.v1.Session().as_default() as sess:
        ...
        # aux1
        re_var = tf.Variable(
            tf.keras.initializers.he_uniform()([1024, num_classes]),
            name="aux1/fc2/kernel",
        )
        new_var_list.append(re_var)
        re_var = tf.Variable(
            tf.keras.initializers.he_uniform()([num_classes]), name="aux1/fc2/bias"
        )
        new_var_list.append(re_var)
        ...
        re_var = tf.Variable(
            tf.keras.initializers.he_uniform()([1024, num_classes]),
            name="aux2/fc2/kernel",
        )
        new_var_list.append(re_var)
        re_var = tf.Variable(
            tf.keras.initializers.he_uniform()([num_classes]), name="aux2/fc2/bias"
        )
        new_var_list.append(re_var)

        # fc
        re_var = tf.Variable(
            tf.keras.initializers.he_uniform()([1024, num_classes]), name="fc/kernel"
        )
        new_var_list.append(re_var)
        re_var = tf.Variable(
            tf.keras.initializers.he_uniform()([num_classes]), name="fc/bias"
        )
        new_var_list.append(re_var)
        ...
        saver.save(
            sess, save_path=new_ckpt_path, write_meta_graph=False, write_state=False
        )
