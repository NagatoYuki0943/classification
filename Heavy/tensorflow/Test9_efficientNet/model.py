...
def block(inputs,                       # 输入的数据
          ...
          use_se: bool = True,
          se_ratio: float = 0.25):      # se 中间减少维度数量se_ratio
    ...
    if use_se:
        ...
        se = layers.GlobalAveragePooling2D(name=name + "se_squeeze")(x)     # [b, h, w, c] => [b, c]
        se = layers.Reshape((1, 1, filters), name=name + "se_reshape")(se)  # [b, c] => [b, 1, 1, c]
        se = layers.Conv2D(filters=filters_se,  # 降维,只需要写out_channel
                           kernel_size=1,
                           padding="same",
                           activation=activation,
                           kernel_initializer=CONV_KERNEL_INITIALIZER,
                           name=name + "se_reduce")(se)
        se = layers.Conv2D(filters=filters,     # 将channel编程原来的channel
                           kernel_size=1,
                           padding="same",
                           activation="sigmoid",
                           kernel_initializer=CONV_KERNEL_INITIALIZER,
...
def efficient_net(width_coefficient,    # 宽度倍率
    ...
    for i, args in enumerate(block_args):
        ...
        for j in range(round_repeats(args[1])):
            x = block(x,
                      activation=activation,
                      drop_rate=drop_connect_rate * b / num_blocks, # drop_rate逐渐升高 b逐渐增大,其他值不变
                      name="block{}{}_".format(i + 1, chr(j + 97)), # block名称 block1a block1b
                      kernel_size=args[0],
                      input_channel=args[2] if j == 0 else args[3], # 第一次会调整out_channel,所以后面的输入全是out_channel
