...
def main(weights_path: str, model_name: str, model: tf.keras.Model):
    ...
    for i in range(model.depth):
        q_kernel = w_dict.pop(
            "encoderblock_{}/MultiHeadAttention/query/kernel".format(i)
        )
        k_kernel = w_dict.pop("encoderblock_{}/MultiHeadAttention/key/kernel".format(i))
        v_kernel = w_dict.pop(
            "encoderblock_{}/MultiHeadAttention/value/kernel".format(i)
        )
        ...
        if model.qkv_bias:
            q_bias = w_dict.pop(
                "encoderblock_{}/MultiHeadAttention/query/bias".format(i)
            )
            k_bias = w_dict.pop("encoderblock_{}/MultiHeadAttention/key/bias".format(i))
            v_bias = w_dict.pop(
                "encoderblock_{}/MultiHeadAttention/value/bias".format(i)
            )
...
if __name__ == "__main__":
    ...
    main(weights_path="./ViT-B_16.npz", model_name="ViT-B_16", model=model)
