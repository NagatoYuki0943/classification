...
class ConcatClassTokenAddPosEmbed(layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        cls_token = tf.broadcast_to(
            self.cls_token, shape=[batch_size, 1, self.embed_dim]
        )
...
class Attention(layers.Layer):
    ...
    def __init__(
        ...
        self.proj = layers.Dense(
            dim, name="out", kernel_initializer=self.k_ini, bias_initializer=self.b_ini
        )
...
class Block(layers.Layer):
    def __init__(
        ...
        self.drop_path = (
            layers.Dropout(rate=drop_path_ratio, noise_shape=(None, 1, 1))
...
class VisionTransformer(Model):
    def __init__(
        ...
        self.cls_token_pos_embed = ConcatClassTokenAddPosEmbed(
            embed_dim=embed_dim, num_patches=num_patches, name="cls_pos"
        )

        self.pos_drop = layers.Dropout(drop_ratio)

        dpr = np.linspace(0.0, drop_path_ratio, depth)  # stochastic depth decay rule
        self.blocks = [
            Block(
                dim=embed_dim,
                num_heads=num_heads,
        ...
        self.head = layers.Dense(
            num_classes, name="head", kernel_initializer=initializers.Zeros()
        )
