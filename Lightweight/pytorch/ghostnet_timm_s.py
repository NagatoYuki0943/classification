...
class GhostModule(nn.Module):
    def __init__(
        ...
        self.primary_conv = nn.Sequential(
            nn.Conv2d(
                in_chs, init_chs, kernel_size, stride, kernel_size // 2, bias=False
            ),
        ...
        self.cheap_operation = nn.Sequential(
            nn.Conv2d(
                init_chs, new_chs, dw_size, 1, dw_size // 2, groups=init_chs, bias=False
            ),
    ...
    def forward(self, x):
        ...
        out = torch.cat(
            [x1, x2], dim=1
        )  # [B, 8, 112, 112] cat [B, 8, 112, 112] = [B, 16, 112, 112]
    ...
    def __init__(
        ...
        if mode == "original":
            self.ghost1 = GhostModule(
                in_chs, mid_chs, use_act=True, act_layer=act_layer
            )
        else:
...
class GhostNet(nn.Module):
    def __init__(
        ...
        assert (
            output_stride == 32
        ), "only output_stride==32 is valid, dilation not supported"
        self.cfgs = cfgs
        ...
        self.classifier = (
            Linear(out_chs, num_classes) if num_classes > 0 else nn.Identity()
        )
    ...
    def reset_classifier(self, num_classes, global_pool="avg"):
        ...
        self.classifier = (
            Linear(self.pool_dim, num_classes) if num_classes > 0 else nn.Identity()
        )
...
def ghostnetv2_100(pretrained=False, **kwargs) -> GhostNet:
    ...
    model = _create_ghostnet(
        "ghostnetv2_100", width=1.0, pretrained=pretrained, version="v2", **kwargs
    )
    return model
...
def ghostnetv2_130(pretrained=False, **kwargs) -> GhostNet:
    """GhostNetV2-1.3x"""
    model = _create_ghostnet(
        "ghostnetv2_130", width=1.3, pretrained=pretrained, version="v2", **kwargs
    )
    return model
...
def ghostnetv2_160(pretrained=False, **kwargs) -> GhostNet:
    """GhostNetV2-1.6x"""
    model = _create_ghostnet(
        "ghostnetv2_160", width=1.6, pretrained=pretrained, version="v2", **kwargs
    )
    return model
