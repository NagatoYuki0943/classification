...
#---------------------------------------------------#
#   LayerScale代替SE
#   参考poolformer, resmlp, coat, efficientformer, maxvit, mobilevit
#---------------------------------------------------#
class LayerScale(nn.Module):
    def __init__(self, dim: int, init_values=1): # init_values=1e-5 -> 1
        super().__init__()
        self.layer_scale = nn.Parameter(init_values * torch.ones(1, dim, 1, 1))
...
class InvertedResidual(nn.Module):
    def __init__(self,
        ...
        if cnf.use_se:                      # 参数是上层DW输出的维度,efficientv1中是开始输入的维度
            layers.append(LayerScale(cnf.expanded_c))
...
def mobilenet_v3_small(pretrained: bool = False, num_classes: int = 1000, reduced_tail: bool = False) -> MobileNetV3:
    ...
    inverted_residual_setting = [
        ...
        bneck_conf(48, 5, 288, 96 // reduce_divider, True, "HS", 2),  # C4 14, 14, 40 -> 7, 7, 96
