...
class InvertedResidualConfig:
    def __init__(self,
                ...
                activation:  str,   # 激活函数
                stride:      int,   # DW卷积步长
                width_multi: float):# alpha参数,调整每个通道的倍率因子
        # 调整倍率
        ...
        self.use_se     = use_se
        self.use_hs     = activation == "HS"  # whether using h-swish activation   使用HS或者ReLU 是一个bool
        self.stride     = stride
...
class InvertedResidual(nn.Module):
    def __init__(self,
        ...
        if cnf.expanded_c != cnf.input_c:
            # 扩展维度
            layers.append(ConvNormActivation(cnf.input_c,                       # in_channel
        ...
        # depthwise DW卷积
        layers.append(ConvNormActivation(cnf.expanded_c,  # in_channel == out_channel == group
                                        cnf.expanded_c,
...
class MobileNetV3(nn.Module):
    def __init__(self,
        ...
        layers.append(ConvNormActivation(3,                   # rgb三通道
                                        firstconv_output_c,
                                        kernel_size     =3,
                                        stride          =2,   # 步长为2
                                        norm_layer      =norm_layer,
...
model_urls = {
    "mobilenet_v3_large": "https://download.pytorch.org/models/mobilenet_v3_large-5c1a4163.pth",    # v2
...
def mobilenet_v3_large(pretrained: bool = False, num_classes: int = 1000, reduced_tail: bool = False) -> MobileNetV3:
    ...
    inverted_residual_setting = [
        ...
        bneck_conf(24,  5, 72,  40, False,  "RE", 2),    # C2
        bneck_conf(40,  5, 120, 40, False,  "RE", 1),    # 56, 56, 24 -> 28, 28, 40
        bneck_conf(40,  5, 120, 40, False,  "RE", 1),
        ...
        bneck_conf(80,  3, 480, 112, False, "HS", 1),    # 14, 14, 80 -> 14, 14, 112
        bneck_conf(112, 3, 672, 112, False, "HS", 1),

        bneck_conf(112, 5, 672, 160 // reduce_divider, False, "HS", 2),  # C4 14, 14, 80 -> 7, 7, 160
        bneck_conf(160 // reduce_divider, 5, 960 // reduce_divider, 160 // reduce_divider, False, "HS", 1),
        bneck_conf(160 // reduce_divider, 5, 960 // reduce_divider, 160 // reduce_divider, False, "HS", 1),
    ]
    last_channel = adjust_channels(1280 // reduce_divider)  # C5 7, 7, 160 -> 7, 7, 1280

    model = MobileNetV3(inverted_residual_setting=inverted_residual_setting, last_channel=last_channel, num_classes=num_classes)
    if pretrained:
...
def mobilenet_v3_small(pretrained: bool = False, num_classes: int = 1000, reduced_tail: bool = False) -> MobileNetV3:
    """
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        num_classes (int): number of classes
    ...
    inverted_residual_setting = [
        ...
        bneck_conf(16, 3, 16, 16, False, "RE", 2),   # C1 112,112,16 -> 56, 56, 16

        bneck_conf(16, 3, 72, 24, False, "RE", 2),  # C2
        bneck_conf(24, 3, 88, 24, False, "RE", 1),  # 56, 56, 16 -> 28, 28, 24

        bneck_conf(24, 5, 96, 40, False, "HS", 2),   # C3
        bneck_conf(40, 5, 240, 40, False, "HS", 1),  # 28, 28, 24 -> 14, 14, 40
        bneck_conf(40, 5, 240, 40, False, "HS", 1),
        bneck_conf(40, 5, 120, 48, False, "HS", 1),
        bneck_conf(48, 5, 144, 48, False, "HS", 1),

        bneck_conf(48, 5, 288, 96 // reduce_divider, False, "HS", 2),  # C4 14, 14, 40 -> 7, 7, 96
        bneck_conf(96 // reduce_divider, 5, 576 // reduce_divider, 96 // reduce_divider, False, "HS", 1),
        bneck_conf(96 // reduce_divider, 5, 576 // reduce_divider, 96 // reduce_divider, False, "HS", 1)
    ]
