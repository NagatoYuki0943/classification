...
class Conv2dBN(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel_size=3, stride=1, groups=1) -> None:
        super().__init__()
...
class Downsample(nn.Module):
    def __init__(self, in_channels, out_channels) -> None:
        ...
        self.avgpool = nn.AvgPool2d(2, ceil_mode=True) # ceil_mode=True: fix the size mismatch in x and y
    ...
    def forward(self, inputs):
        ...
        b = torch.mul(a, z)    # [1, 64, 112, 112] * [1, 64, 1, 1] = [1, 64, 112, 112]      a * z channel attn
        c = self.activation(b)
        return c
...
class FuseBlock(nn.Module):
    def __init__(self, in_channels, out_channels, inference_mode=False) -> None:
        ...
        if self.inference_mode:
            self.reparam_conv = nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1, bias=True)
        else:
    ...
    def reparameterize(self):
        self.reparam_conv = nn.Conv2d(self.in_channels, self.out_channels, kernel_size=3, padding=1, bias=True)
...
class SSEBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        z = torch.mul(bn, x)        # [B, 96, 56, 56] * [B, 96, 1, 1] = [B, 96, 56, 56]     bn * x channel attn
        return z
...
class Stream(nn.Module):
    def __init__(self, in_channels, out_channels, inference_mode=False) -> None:
        ...
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.fuse = FuseBlock(self.in_channels, self.out_channels, inference_mode=inference_mode)
...
class Fusion(nn.Module):
    def __init__(self, in_channels, out_channels) -> None:
        ...
        self.conv1 = Conv2dBN(self.network_in_channels, out_channels, kernel_size=1, groups=2)
        self.conv2 = Conv2dBN(self.network_in_channels, out_channels, kernel_size=3, stride=2, groups=2)
        self.conv3 = nn.Conv2d(self.network_in_channels, out_channels, 1, groups=2, bias=False)
    ...
    def forward(self, input1, input2):
        ...
        a_ = torch.cat([self.bn(input1), self.bn(input2)], dim=1)    # [B, 192, 28, 28] cat [B, 192, 28, 28] = [B, 384, 28, 28]
        ...
        b = torch.mul(a, z)         # [B, 384, 14, 14] * [B, 384, 1, 1] = [B, 384, 14, 14]  a * z channel attn
        c = self.activation(b)
        return c


class ParNetEncoder(nn.Module):
    def __init__(self, in_channels, block_size, depth=[4, 5, 5], inference_mode=False) -> None:
        super().__init__()
        self.in_channels = in_channels
        ...
        self.stream1 = nn.Sequential(
            *[Stream(self.block_size[1], self.block_size[1], inference_mode=inference_mode) for _ in range(self.depth[0])]
        )
        ...
        self.stream2 = nn.Sequential(
            *[Stream(self.block_size[2], self.block_size[2], inference_mode=inference_mode) for _ in range(self.depth[1])]
        )

        # 5层
        self.stream3 = nn.Sequential(
            *[Stream(self.block_size[3], self.block_size[3], inference_mode=inference_mode) for _ in range(self.depth[2])]
        )
    ...
    def forward(self, inputs):
        ...
        z = self.stream2_fusion(y, z)   # [B, 192, 28, 28] and [B, 192, 28, 28] -> [B, 384, 14, 14]

        x = self.d4(x)                  # [B, 192, 28, 28] -> [B, 384, 14, 14]

        a = self.stream3(x)             # [B, 384, 14, 14] -> [B, 384, 14, 14]
        b = self.stream3_fusion(z, a)   # [B, 384, 14, 14] and [B, 384, 14, 14] = [B, 768, 7, 7]
