    ...
    def __init__(self, input_c: int, output_c: int, stride: int):
        ...
        if self.stride == 2:
            ...
            self.branch1 = nn.Sequential(
                # 输入维度就是in_channel
                self.depthwise_conv(input_c, input_c, kernel_s=3, stride=self.stride, padding=1),       # DW卷积,输入和输出不变 3x3
                nn.BatchNorm2d(input_c),

                nn.Conv2d(input_c, branch_features, kernel_size=1, stride=1, padding=0, bias=False),    # 1x1
        ...
        self.branch2 = nn.Sequential(
            ...
            nn.Conv2d(input_c if self.stride > 1 else branch_features, branch_features, kernel_size=1, stride=1, padding=0, bias=False),    # 1x1
            nn.BatchNorm2d(branch_features),
            nn.ReLU(inplace=True),

            self.depthwise_conv(branch_features, branch_features, kernel_s=3, stride=self.stride, padding=1),   # 3x3
            nn.BatchNorm2d(branch_features),

            nn.Conv2d(branch_features, branch_features, kernel_size=1, stride=1, padding=0, bias=False),        # 1x1
            nn.BatchNorm2d(branch_features),
            nn.ReLU(inplace=True)
        )
...
class ShuffleNetV2(nn.Module):
    def __init__(self,
                ...
                stages_out_channels: List[int],    # conv1 stage2 3 4 5 的输出维度(每个阶段的第一个卷积就变化)
                num_classes: int = 1000,           # 分类
                inverted_residual: Callable[..., nn.Module] = InvertedResidual):   # 卷积类
        super().__init__()
...
def shufflenet_v2_x0_5(num_classes=1000, pretrained=False):
    ...
    :return:
    """
    model = ShuffleNetV2(stages_repeats=[4, 8, 4],                      # stage2 3 4的重复次数
                        stages_out_channels=[24, 48, 96, 192, 1024],   # conv1 stage2 3 4 5 的输出维度(每个阶段的第一个卷积就变化)
                        num_classes=num_classes)
    if pretrained:
        state_dict = load_state_dict_from_url("https://download.pytorch.org/models/shufflenetv2_x0.5-f707e7126e.pth", progress=True)
...
def shufflenet_v2_x1_0(num_classes=1000, pretrained=False):
    ...
    """
    model = ShuffleNetV2(stages_repeats=[4, 8, 4],                      # stage2 3 4的重复次数
                        stages_out_channels=[24, 116, 232, 464, 1024], # conv1 stage2 3 4 5 的输出维度(每个阶段的第一个卷积就变化)
                        num_classes=num_classes)
    if pretrained:
        state_dict = load_state_dict_from_url("https://download.pytorch.org/models/shufflenetv2_x0.5-5666bf0f80.pth", progress=True)
        model.load_state_dict(state_dict)
    return model
