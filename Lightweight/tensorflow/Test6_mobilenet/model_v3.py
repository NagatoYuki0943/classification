...
def _se_block(inputs,               # 输入数据 [batch, height, width, channel]
                filters,            # 输入数据的channel
                prefix,             # Conv2D名字前缀
                se_ratio=1 / 4.):   # filters缩小倍率
    '''
    注意力机制
    '''
...
def _inverted_res_block(x,                 # data
...
def mobilenet_v3_large(input_shape=(224, 224, 3),   # 图片形状
                       num_classes=1000,
                       alpha=1.0,                   # 超参数
                       include_top=True):           # 分类结构
    """
    ...
    if include_top is True:
        x = layers.GlobalAveragePooling2D()(x)  # [batch, height, width, channel] -> [batch, channel]
        x = layers.Reshape((1, 1, last_c))(x)   # [batch, channel] -> [batch, 1, 1, channel]
