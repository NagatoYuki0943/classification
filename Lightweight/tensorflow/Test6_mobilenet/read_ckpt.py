    ...
    with tf.Graph().as_default(), tf.compat.v1.Session().as_default() as sess:
        ...
        for var_name, shape in var_list:
            ...
            new_var_name = new_var_name.replace(
                "/expand/weights", "/expand/Conv2d/weights"
            )
            new_var_name = new_var_name.replace("Conv/weights", "Conv/Conv2d/kernel")
            new_var_name = new_var_name.replace(
                "Conv_1/weights", "Conv_1/Conv2d/kernel"
            )
        ...
        re_var = tf.Variable(
            tf.keras.initializers.he_uniform()([1280, num_classes]),
            name="Logits/kernel",
        )
        new_var_list.append(re_var)
        re_var = tf.Variable(
            tf.keras.initializers.he_uniform()([num_classes]), name="Logits/bias"
        )
