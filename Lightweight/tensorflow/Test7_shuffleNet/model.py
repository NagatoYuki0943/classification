...
class DWConvBN(layers.Layer):
    ...
    def __init__(
        self, kernel_size: int = 3, strides: int = 1, padding: str = "same", **kwargs
    ):
        super().__init__(**kwargs)
...
def shufflenet_v2(
...
):  # conv1,stage2,3,4,conv5输出维度: [24, 116, 232, 464, 1024]
    ...
    # 开始卷积
    x = ConvBNReLU(
        filters=stages_out_channels[0], kernel_size=3, strides=2, name="conv1"
    )(img_input)
    # 池化
    x = layers.MaxPooling2D(
        pool_size=(3, 3), strides=2, padding="same", name="maxpool"
    )(x)
    ...
    ):  # 第一个已经使用了,略过, 循环3个数据,不过由于上面两个参数只有3个,所以最后一个用不到
        # 每个stage循环stage次
        for i in range(repeats):
            # 第0次改变channel和HW,其他不变(channel变化是相对于之前的数据)
            if i == 0:
                x = shuffle_block_s2(
                    x, output_c=output_channels, stride=2, prefix=name + "_{}".format(i)
                )
            else:
                x = shuffle_block_s1(
                    x, output_c=output_channels, stride=1, prefix=name + "_{}".format(i)
                )
