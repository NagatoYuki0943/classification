...
def main():
    ...
    m_info = [(i.name.replace(":0", ""), list(i.shape)) for i in m.weights]
    ...
    for key, value in weights_dict.items():
        if "conv1.0.weight" == key:
            value = np.transpose(value.detach().numpy(), (2, 3, 1, 0)).astype(
                np.float32
            )
            ...
            torch_name2tf_name = {
                "1.0.weight": "b1_dw1/dw1/depthwise_kernel",
            ...
            if len(value.shape) > 1:  # conv or dwconv
                if "dw" in tf_name:
                    value = np.transpose(value.detach().numpy(), (2, 3, 0, 1)).astype(
                        np.float32
                    )
                else:
                    value = np.transpose(value.detach().numpy(), (2, 3, 1, 0)).astype(
    ...
    for name, shape in m_info:
        ...
        assert (
            tuple(shape) == new_weights_dict[name].shape
        ), "tf shape:'{}', trans shape:'{}'".format(shape, new_weights_dict[name].shape)
