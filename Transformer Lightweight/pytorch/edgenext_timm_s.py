...
from timm.layers import trunc_normal_tf_, DropPath, LayerNorm2d, SelectAdaptivePool2d, create_conv2d, \
    use_fused_attn, to_2tuple
...
class CrossCovarianceAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = qkv.reshape(B, N, 3, self.num_heads, -1)  # [B, N, C*3] -> [B, N, 3, h, c]  C = h * c
        qkv = qkv.permute(2, 0, 3, 4, 1)                # [B, N, 3, h, c] -> [3, B, h, c, N]    这里的 c P 的位置与一般的attn相反   !!!使用了通道的注意力!!! davit也用了这个
        ...
        #---------------------------------------------------------------------------------------#
        attn = (F.normalize(q, dim=-1) @ F.normalize(k, dim=-1).transpose(-2, -1)) * self.temperature   #  [B, h, c, N] @  [B, h, N, c] = [B, h, c, c]
...
class SplitTransposeBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.pos_embd is not None:               # 位置编码 [B, H, W] -> [B, 1, H*W] -> [B, H*W, 1]
            pos_encoding = self.pos_embd((B, H, W)).reshape(B, -1, x.shape[1]).permute(0, 2, 1)
            x = x + pos_encoding                    # [B ,H*W, C] + [B, H*W, 1] = [B ,H*W, C]
        x = x + self.drop_path(self.gamma_xca * self.xca(self.norm_xca(x))) # [B ,H*W, C] -> [B ,H*W, C] 经过多分支卷积,attn,mlp
...
class EdgeNeXtStage(nn.Module):
    ...
    def forward(self, x):       #         第一次没有下采样,stem做了
        x = self.downsample(x)  # [B, 48, 64, 64] -> [B, 48, 64, 64] -> [B, 96, 32, 32] -> [B, 160, 16, 16] -> [B, 304, 8, 8]
...
class EdgeNeXt(nn.Module):
    def __init__(
            ...
            d2_scales=(2, 2, 3, 4),                     # Block中的多分支卷积之前将通道划分为n份,stage1无效,因为没使用SplitTransposeBlock
            use_pos_emb=(False, True, False, False),    # 每个stage中是否使用位置编码,只有stage2使用了
            ls_init_value=1e-6,
            head_init_scale=1.,
            expand_ratio=4,                             # mlp中的扩展倍率
            downsample_block=False,
        ...
        for i in range(4):
            ...
            stages.append(EdgeNeXtStage(
                ...
                stride=stride,
                depth=depths[i],
                num_global_blocks=global_block_counts[i],   # 每个stage中重复SplitTransposeBlock的次数
                ...
                use_pos_emb=use_pos_emb[i],                 # 每个stage中是否使用位置编码,只有stage2使用了
                ls_init_value=ls_init_value,
    ...
    def forward_features(self, x):
        x = self.stem(x)        # [B, 3, 256, 256] -> [B, 48, 64, 64]   stem直接使用 stride=4 的卷积完成
