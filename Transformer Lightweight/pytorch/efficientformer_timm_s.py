...
class LayerScale2d(nn.Module):
    ...
    def forward(self, x):
        ...
        return x.mul_(gamma) if self.inplace else x * gamma # [B, C, H, W] * [1, C, 1, 1] = [B, C, H, W]
...
class MetaBlock2d(nn.Module):
    ...
    def forward(self, x):
        x = x + self.drop_path1(self.ls1(self.token_mixer(x)))  # [B, C, H, W] -> [B, C, H, W]  pooling(k=3)
        x = x + self.drop_path2(self.ls2(self.mlp(x)))          # [B, C, H, W] -> [B, 4*C, H, W] -> [B, C, H, W]
        return x
...
class Attention(torch.nn.Module):
    ...
    def forward(self, x):  # x: [B, N, C]
        ...
        q, k, v = qkv.split([self.key_dim, self.key_dim, self.val_dim], dim=3) # [B, 8, 49, 192] -> [B, 8, 49, 32], [B, 8, 49, 32], [B, 8, 49, 128]

        attn = (q @ k.transpose(-2, -1)) * self.scale       # [B, 8, 49, 32] @ [B, 8, 32, 49] = [B, 8, 49, 49]
...
class MetaBlock1d(nn.Module):
    ...
    def forward(self, x):
        x = x + self.drop_path(self.ls1(self.token_mixer(self.norm1(x))))   # [B, N, C] -> [B, N, C]
        x = x + self.drop_path(self.ls2(self.mlp(self.norm2(x))))           # [B, N, C] -> [B, N, 4*C] -> [B, N, C]
        return x
...
class EfficientFormerStage(nn.Module):
    ...
    def forward(self, x):                  # 第一次没有下采样
        x = self.downsample(x)  # [B, 48, 56, 56] -> [B, 48, 56, 56] -> [B, 96, 28, 28] -> [B, 224, 14, 14] -> [B, 448, 7, 7]
...
class EfficientFormer(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(depths)):
            stage = EfficientFormerStage(
                ...
                num_vit=num_vit if i == 3 else 0,   # 最后的stage使用attn, 参数是使用 attn block 的次数
                pool_size=pool_size,
    ...
    def forward_features(self, x):
        ...
        x = self.stages(x)  # [B, 48, 56, 56] -> [B, 96, 28, 28] -> [B, 224, 14, 14] -> [B, 49, 448]
        x = self.norm(x)
        return x
    ...
    def forward_head(self, x, pre_logits: bool = False):
        ...
        if pre_logits:
            return x
        x, x_dist = self.head(x), self.head_dist(x) # [B, 448] -> [B, num_classes] and [B, 448] -> [B, num_classes]
