...
class Attention2d(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        k = k.permute(0, 1, 2, 3)                   # [B, 8, 32, 49] -> [B, 8, 32, 49] are you kidding me?

        v = self.v(x)                               # [B, 96, 7, 7]   -> [B, 1024, 7, 7]
        v_local = self.v_local(v)                   # [B, 1024, 7, 7] -> [B, 1024, 7, 7]
        v = v.reshape(B, self.num_heads, -1, self.N)# [B, 1024, 7, 7] -> [B, 8, 128, 49]
        v = v.permute(0, 1, 3, 2)                   # [B, 8, 128, 49] -> [B, 8, 49, 128]

        attn = (q @ k) * self.scale                 # [B, 8, 49, 32] @ [B, 8, 32, 49] = [B, 8, 49, 49]
        attn = attn + self.get_attention_biases(x.device) # [B, 8, 49, 49] + [8, 49, 49] = [B, 8, 49, 49]
...
class Attention2dDownsample(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        q = q.reshape(B, self.num_heads, -1, self.N2)   # [B, 128, 7, 7] -> [B, 8, 16, 49]
        q = q.permute(0, 1, 3, 2)                       # [B, 8, 16, 49] -> [B, 8, 49, 16]

        k = self.k(x)                                   # [B, 96, 14, 14] -> [B, 128, 14, 14]
        k = k.reshape(B, self.num_heads, -1, self.N)    # [B, 128, 14, 14] -> [B, 8, 16, 196]
        k = k.permute(0, 1, 2, 3)                       # [B, 8, 16, 196] -> [B, 8, 16, 196] are you kidding me?

        v = self.v(x)                                   # [B, 96, 14, 14] -> [B, 512, 14, 14]
        v_local = self.v_local(v)                       # [B, 512, 14, 14] -> [B, 512, 7, 7]
        v = v.reshape(B, self.num_heads, -1, self.N)    # [B, 512, 14, 14] -> [B, 8, 64, 196]
        v = v.permute(0, 1, 3, 2)                       # [B, 8, 64, 196] -> [B, 8, 196, 64]

        attn = (q @ k) * self.scale                     # [B, 8, 49, 16] @ [B, 8, 16, 196] = [B, 8, 49, 196]
        attn = attn + self.get_attention_biases(x.device)# [B, 8, 49, 196] + [8, 49, 196] = [B, 8, 49, 196]
        attn = attn.softmax(dim=-1)                     # 在行上做softmax
...
class Downsample(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.attn is not None:       # stage4使用self attn
            return self.attn(x) + out   # [B, 176, 7, 7] + [B, 176, 7, 7] = [B, 176, 7, 7]
        return out
...
class EfficientFormerV2Block(nn.Module):
    ...
    def forward(self, x):
        if self.token_mixer is not None:    # 前2个stage没有用mixer,只有mlp,后2个stage每个stage前面也只用mlp,后面使用mixer
...
class EfficientFormerV2Stage(nn.Module):
    ...
    def __init__(
        ...
        for block_idx in range(depth):
            ...
            b = EfficientFormerV2Block(
                ...
                use_attn=block_use_attn and block_idx > remain_idx,    # stage最后才使用vit
                proj_drop=proj_drop,
    ...
    def forward(self, x):                  # 第一次没有下采样
        x = self.downsample(x)  # [B, 32, 56, 56] -> [B, 32, 56, 56] -> [B, 48, 28, 28] -> [B, 96, 14, 14] -> [B, 176, 7, 7]
...
class EfficientFormerV2(nn.Module):
    ...
    def forward_head(self, x, pre_logits: bool = False):
        ...
        if pre_logits:
            return x
        x, x_dist = self.head(x), self.head_dist(x) # [B, 176] -> [B, num_classes] and [B, 176] -> [B, num_classes]
        if self.distilled_training and self.training and not torch.jit.is_scripting():
            # only return separate classification predictions when training in distilled mode
            return x, x_dist
        else:
            # during standard train/finetune, inference average the classifier predictions
            return (x + x_dist) / 2     # ([B, num_classes] + [B, num_classes]) / 2 = [B, num_classes]
