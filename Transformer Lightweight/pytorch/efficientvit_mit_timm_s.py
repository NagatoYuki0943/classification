...
class LiteMLA(nn.Module):
    ...
    def _attn(self, q, k, v):
        ...
        kv = (
            k.transpose(-1, -2) @ v
        )  # [B, 8, 196, 16] -> [B, 8, 16, 196] @ [B, 8, 196, 17] = [B, 8, 16, 17]
        out = q @ kv  # [B, 8, 196, 16] @ [B, 8, 16, 17] = [B, 8, 196, 17]
        out = out[..., :-1] / (
            out[..., -1:] + self.eps
        )  # [B, 8, 196, 17] get [B, 8, 196, 16]
        return out.to(dtype)
    ...
    def forward(self, x):
        ...
        multi_scale_qkv = torch.cat(
            multi_scale_qkv, dim=1
        )  # 拼接多尺度的qkv   2 * [B, 3*64, 14, 14] -> [B, 6*64, 14, 14]
        multi_scale_qkv = multi_scale_qkv.reshape(B, -1, 3 * self.dim, H * W).transpose(
            -1, -2
        )  # [B, 6*64, 14, 14] -> [B, 8, 48, 196] -> [B, 8, 196, 48]
        q, k, v = multi_scale_qkv.chunk(
            3, dim=-1
        )  # [B, 8, 196, 48] -> 3 * [B, 8, 196, 16]

        # lightweight global attention
        q = self.kernel_func(q)  # ReLU [B, 8, 196, 16] -> [B, 8, 196, 16]
        k = self.kernel_func(k)  # ReLU [B, 8, 196, 16] -> [B, 8, 196, 16]
        v = F.pad(
            v, (0, 1), mode="constant", value=1.0
        )  # [B, 8, 196, 16] -> [B, 8, 196, 17]
        ...
        out = out.transpose(-1, -2).reshape(
            B, -1, H, W
        )  # [B, 8, 196, 16] -> [B, 8, 16, 196] -> [B, 128, 14, 14]
...
class Stem(nn.Sequential):
    def __init__(
        self, in_chs, out_chs, depth, norm_layer, act_layer, block_type="default"
    ):
        super().__init__()
        self.stride = 2
    ...
    def __init__(
        ...
        if vit_stage:
            ...
            for i in range(1, depth):
                blocks.append(
                    ResidualBlock(  # DSConv(3x3DWConv+1x1PWConv)
                        build_local_block(
                            in_channels=in_chs,
                            out_channels=out_chs,
                            stride=1,
    ...
    def __init__(
        ...
        if vit_stage:
            ...
            for i in range(depth):
                blocks.append(
                    ResidualBlock(  # ConvBlock(3x3Conv+3x3Conv)
                        build_local_block(
                            in_channels=in_chs,
                            out_channels=out_chs,
                            stride=1,
...
class ClassifierHead(nn.Module):
    def __init__(
        ...
        self.global_pool = SelectAdaptivePool2d(
            pool_type=global_pool, flatten=True, input_fmt="NCHW"
        )
...
class EfficientVit(nn.Module):
    ...
    def reset_classifier(self, num_classes, global_pool=None):
        ...
        if num_classes > 0:
            ...
            if self.global_pool == "avg":
                self.head = SelectAdaptivePool2d(
                    pool_type=self.global_pool, flatten=True
                )
            else:
                self.head = nn.Identity()
    ...
    def forward_head(self, x, pre_logits: bool = False):
        return (
            self.head(x, pre_logits=pre_logits) if pre_logits else self.head(x)
        )  # [B, 128, 7, 7] -> [B, num_classes]
...
def efficientvit_b0(pretrained=False, **kwargs):
    ...
    return _create_efficientvit(
        "efficientvit_b0", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def efficientvit_b1(pretrained=False, **kwargs):
    ...
    return _create_efficientvit(
        "efficientvit_b1", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def efficientvit_b2(pretrained=False, **kwargs):
    ...
    return _create_efficientvit(
        "efficientvit_b2", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def efficientvit_b3(pretrained=False, **kwargs):
    ...
    return _create_efficientvit(
        "efficientvit_b3", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def efficientvit_l1(pretrained=False, **kwargs):
    ...
    return _create_efficientvit_large(
        "efficientvit_l1", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def efficientvit_l2(pretrained=False, **kwargs):
    ...
    return _create_efficientvit_large(
        "efficientvit_l2", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def efficientvit_l3(pretrained=False, **kwargs):
    ...
    return _create_efficientvit_large(
        "efficientvit_l3", pretrained=pretrained, **dict(model_args, **kwargs)
    )
