...
class CascadedGroupAttention(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        feats_in = x.chunk(len(self.qkvs), dim=1)               # [B, C, H, W] -> n * [B, c, H, W]  C = n * c
        feats_out = []
        ...
        for head_idx, (qkv, dws) in enumerate(zip(self.qkvs, self.dws)):
            ...
            feat = qkv(feat)                                    # [B, c, H, W] -> [B, 3*c, H, W] 不一定是3*c,q和k一定相等,v不一定
            q, k, v = feat.view(B, -1, H, W).split([self.key_dim, self.key_dim, self.val_dim], dim=1)   # [B, 3*c, H, W] -> 3 * [B, c, H, W]
            q = dws(q)                                          # [B, c, H, W] -> [B, c, H, W]  Token Interaction(DWConvNorm)
            q, k, v = q.flatten(2), k.flatten(2), v.flatten(2)  # [B, c, H, W] -> [B, c, H*W]
            q = q * self.scale
            attn = q.transpose(-2, -1) @ k                      # ([B, c, H*W] -> [B, H*W, c]) @ [B, c, H*W] = [B, H*W, H*W]
            attn = attn + attn_bias[head_idx]                   # [B, H*W, H*W] + [1, H*W, H*W] = [B, H*W, H*W]
            attn = attn.softmax(dim=-1)
            feat = v @ attn.transpose(-2, -1)                   # [B, c, H*W] @ ([B, H*W, H*W] -> [B, H*W, H*W]) = [B, c, H*W]
            feat = feat.view(B, self.val_dim, H, W)             # [B, c, H*W] -> [B, c, H, W]
            feats_out.append(feat)
        x = self.proj(torch.cat(feats_out, 1))                  # n * [B, c, H, W] cat [B, C, H, W] -> [B, C, H, W]
        return x
...
class EfficientVitStage(nn.Module):
    ...
    def forward(self, x):       #           第一次不用下采样
        x = self.downsample(x)  # [B, 64, 14, 14] -> [B, 64, 14, 14] -> [B, 128, 7, 7] -> [B, 192, 4, 4]
        x = self.blocks(x)
        return x
...
class EfficientVitMsra(nn.Module):
    ...
    def forward_features(self, x):
        x = self.patch_embed(x) # [B, 3, 224, 224] -> [B, 64, 14, 14] 直接下采样16倍,类似 vit_patch16
