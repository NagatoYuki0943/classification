...
from timm.layers import to_2tuple, make_divisible, GroupNorm1, DropPath, is_exportable, use_fused_attn
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = qkv.reshape(B, N, 3, self.num_heads, self.head_dim)   # [B, N, 3*C] -> [B, N, 3, h, c]    C = h * c
...
class MobileVitBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.reshape(B * C * num_patch_h, patch_h, num_patch_w, patch_w)   # [B, 144, 32, 32] -> [B*144*16, 2, 16, 2]
        x = x.transpose(1, 2)                                               # [B*144*16, 2, 16, 2] -> [B*144*16, 16, 2, 2]

        # [B * C * n_h, n_w, p_h, p_w] --> [BP, N, C] where P = p_h * p_w and N = n_h * n_w
        x = x.reshape(B, C, num_patches, self.patch_area)                   # [B*144*16, 16, 2, 2] -> [B, 144, 16*16, 2*2]
        x = x.transpose(1, 3)                                               # [B, 144, 16*16, 2*2] -> [B, 2*2, 16*16, 144]
        x = x.reshape(B * self.patch_area, num_patches, -1)                 # [B, 2*2, 16*16, 144] -> [B*2*2, 16*16, 144]
        ...
        x = x.contiguous().view(B, self.patch_area, num_patches, -1)        # [B*2*2, 16*16, 144] -> [B, 2*2, 16*16, 144]
        x = x.transpose(1, 3)                                               # [B, 2*2, 16*16, 144] -> [B, 144, 16*16, 2*2]
        x = x.reshape(B * C * num_patch_h, num_patch_w, patch_h, patch_w)   # [B, 144, 16*16, 2*2] -> [B*144*16, 16, 2, 2]

        # [B*C*n_h, n_w, p_h, p_w] --> [B*C*n_h, p_h, n_w, p_w] --> [B, C, H, W]
        x = x.transpose(1, 2)                                               # [B*144*16, 16, 2, 2] -> [B*144*16, 2, 16, 2]
        x = x.reshape(B, C, num_patch_h * patch_h, num_patch_w * patch_w)   # [B*144*16, 2, 16, 2] -> [B, 144, 32, 32]
...
class LinearSelfAttention(nn.Module):
    ...
    def _forward_self_attn(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Query --> [B, 1, P, N]
        # value, key --> [B, d, P, N]
        query, key, value = qkv.split([1, self.embed_dim, self.embed_dim], dim=1)   # [B, 257, 4, 256] -> [B, 1, 4, 256], [B, 128, 4, 256], [B, 128, 4, 256]
        ...
        context_vector = (key * context_scores).sum(dim=-1, keepdim=True)   # [B, 128, 4, 256] * [B, 1, 4, 256] = [B, 128, 4, 256] -> [B, 128, 4, 1]

        # combine context vector with values
        # [B, d, P, N] * [B, d, P, 1] --> [B, d, P, N]
        out = F.relu(value) * context_vector.expand_as(value)   # [B, 128, 4, 256] * [B, 128, 4, 1] = [B, 128, 4, 256]
...
class LinearTransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, x_prev: Optional[torch.Tensor] = None) -> torch.Tensor:
        if x_prev is None:
            # self-attention
            x = x + self.drop_path1(self.attn(self.norm1(x))) # [B, 128, 4, 256] -> [B, 128, 4, 256]
        else:
            # cross-attention
            res = x
            x = self.norm1(x)  # norm
        ...
        x = x + self.drop_path2(self.mlp(self.norm2(x)))    # [B, 128, 4, 256] -> [B, 2*128, 4, 256] -> [B, 128, 4, 256]
        return x
...
class MobileVitV2Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.coreml_exportable:
            ...
            x = x.reshape(B, C, num_patch_h, patch_h, num_patch_w, patch_w)     # [B, 128, 32, 32] -> [B, 128, 16, 2, 16, 2]
            x = x.permute(0, 1, 3, 5, 2, 4)                                     # [B, 128, 16, 2, 16, 2] -> [B, 128, 2, 2, 16, 16]
        x = x.reshape(B, C, -1, num_patches)                                    # [B, 128, 2, 2, 16, 16] -> [B, 128, 2*2, 16*16]
        ...
        if self.coreml_exportable:
            ...
            x = x.reshape(B, C, patch_h, patch_w, num_patch_h, num_patch_w)     # [B, 128, 2*2, 16*16] -> [B, 128, 2, 2, 16, 16]
            x = x.permute(0, 1, 4, 2, 5, 3)                                     # [B, 128, 2, 2, 16, 16] -> [B, 128, 16, 2, 16, 2]
            x = x.reshape(B, C, num_patch_h * patch_h, num_patch_w * patch_w)   # [B, 128, 16, 2, 16, 2] -> [B, 128, 32, 32]
...
from timm.models.byobnet import get_layer_fns, create_byob_stem, reduce_feat_size, create_byob_stages, _init_weights
