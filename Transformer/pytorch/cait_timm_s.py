...
class ClassAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        q = q.reshape(B, 1, self.num_heads, C // self.num_heads)    # [B, 1, C] -> [B, 1, h, c]  h * c = C
        q = q.permute(0, 2, 1, 3)                                   # [B, 1, h, c] -> [B, h, 1, c]

        k = self.k(x)                                               # [B, N, C] -> [B, N, C]        k和v的 P 为197,包含cls层,只有q只有cls一层
        k = k.reshape(B, N, self.num_heads, C // self.num_heads)    # [B, N, C] -> [B, N, h, c]
        k = k.permute(0, 2, 1, 3)                                   # [B, N, h, c] -> [B, h, N, c]

        v = self.v(x)                                               # [B, N, C] -> [B, N, C]
        v = v.reshape(B, N, self.num_heads, C // self.num_heads)    # [B, N, C] -> [B, N, h, c]
...
class TalkingHeadAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = qkv.reshape(B, N, 3, self.num_heads, C // self.num_heads) # [B, N, C*3] -> [B, N, 3, h, c]    h * c = C
        qkv = qkv.permute(2, 0, 3, 1, 4)                                # [B, N, 3, h, c] -> [3, B, h, N, c]
        q, k, v = qkv[0] * self.scale, qkv[1], qkv[2]                   # [3, B, h, N, c] -> 3 * [B, h, N, c]

        attn = (q @ k.transpose(-2, -1))                # [B, h, N, c] @ [B, h, c, N] = [B, h, N, N]

        # cait独有的
        attn = self.proj_l(attn.permute(0, 2, 3, 1))    # [B, h, N, N] -> [B, N, N, h] -> [B, N, N, h] 对每个head进行投影
        attn = attn.permute(0, 3, 1, 2)                 # [B, N, N, h] -> [B, h, N, N]

        attn = attn.softmax(dim=-1)                     # 取每一列,在行上做softmax

        # cait独有的
        attn = self.proj_w(attn.permute(0, 2, 3, 1))    # [B, h, N, N] -> [B, N, N, h] -> [B, N, N, h] 对每个head进行投影
        attn = attn.permute(0, 3, 1, 2)                 # [B, N, N, h] -> [B, h, N, N]
        attn = self.attn_drop(attn)
...
class LayerScaleBlock(nn.Module):
    ...
    def forward(self, x):
        # 使用LayerScale
        x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x))) # [192] * [B, 196, 192] = [B, 196, 192]
        x = x + self.drop_path(self.gamma_2 * self.mlp(self.norm2(x)))  # [192] * [B, 196, 192] = [B, 196, 192]
        return x
...
class Cait(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim))   # [B, 196, 192]
    ...
    def forward_features(self, x):
        x = self.patch_embed(x) # [B, 3, 224, 224] -> [B, 192, 14, 14] -> [B, 192, 14*14] -> [B, 196, 192]
        ...
        cls_tokens = self.cls_token.expand(x.shape[0], -1, -1)  # [1, 1, 192] -> [B, 1, 192]  expand 在指定维度复制n份

        # 只有2层 分类层在最后才添加
        for i, blk in enumerate(self.blocks_token_only):
            cls_tokens = blk(x, cls_tokens)     # [B, 196, 192] with [B, 1, 192] -> [B, 1, 192] 这里的x的两层的输入的是同一个值
        x = torch.cat((cls_tokens, x), dim=1)   # [B, 1, 192] cat [B, 196, 192] = [B, 197, 192]
        x = self.norm(x)
        return x

    def forward_head(self, x, pre_logits: bool = False):
        if self.global_pool:
            x = x[:, 1:].mean(dim=1) if self.global_pool == 'avg' else x[:, 0]  # [B, 197, 192] get cls [B, 192]
        x = self.head_drop(x)
