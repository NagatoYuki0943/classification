...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.flatten:
            ...
            x = nchw_to(x, self.output_fmt)   # [B, 64, 56, 56] -> [B, 64, 56*56] -> [B, 56*56, 64]
        x = self.norm(x)
        return x
...
class ConvPosEnc(nn.Module):
    ...
    def forward(self, x, size: Tuple[int, int]):
        ...
        feat = img_tokens.transpose(1, 2).view(B, C, H, W)  # [B, H*W, C] -> [B, C, H*W] -> [B, C, H, W]
        x = self.proj(feat) + feat                          # [B, C, H, W] + [B, C, H, W] = [B, C, H, W]
        x = x.flatten(2).transpose(1, 2)                    # [B, C, H, W] -> [B, C, H*W] -> [B, H*W, C]

        # Combine with CLS token.
        x = torch.cat((cls_token, x), dim=1)                # [B, 1, C] cat [B, H*W, C] = [B, H*W+1, C]
...
class ConvRelPosEnc(nn.Module):
    ...
    def forward(self, q, v, size: Tuple[int, int]):
        ...
        conv_v_img = conv_v_img.reshape(B, num_heads, C, H * W)     # [B, C, H, W]- -> [B, h, c, H*W]
...
class FactorAttnConvRelPosEnc(nn.Module):
    ...
    def forward(self, x, size: Tuple[int, int]):
        ...
        qkv = qkv.reshape(B, N, 3, self.num_heads, C // self.num_heads) # [B, N, C*3] -> [B, N, 3, h, c]
        qkv = qkv.permute(2, 0, 3, 1, 4)                                # [B, N, 3, h, c] -> [3, B, h, N, c]
        q, k, v = qkv.unbind(0)                                         # [3, B, h, N, c] -> [B, h, N, c] * 3

        # Factorized attention.
        k_softmax = k.softmax(dim=2)                    # [B, h, N, c] 在 position上做softmax,下面的转置回到最后维度
        factor_att = k_softmax.transpose(-1, -2) @ v    # [B, h, c, N] @ [B, h, N, c] = [B, h, c, c]
        ...
        # Merge and reshape.
        x = self.scale * factor_att + crpe      # num * [B, h, N, c] + [B, h, N, c] = [B, h, N, c]
...
class ParallelBlock(nn.Module):
    ...
    def interpolate(self, x, scale_factor: float, size: Tuple[int, int]):
        ...
        img_tokens = img_tokens.transpose(1, 2).reshape(B, C, H, W) # [B, H*W, C] -> [B, C, H*W] -> [B, C, H, W]
        ...
        img_tokens = img_tokens.reshape(B, C, -1).transpose(1, 2)   # [B, C, nH, nW] -> [B, C, nH*nW] -> [B, nH*nW, C]

        out = torch.cat((cls_token, img_tokens), dim=1)             # [B, 1, C] cat [B, nH*nW, C] = [B, nH*nW+1, C]
    ...
    def forward(self, x1, x2, x3, x4, sizes: List[Tuple[int, int]]):
        ...
        upsample3_2 = self.upsample(cur3, factor=2., size=S3)       # [B, 197, 152] -> [B, 785, 152]
        upsample4_3 = self.upsample(cur4, factor=2., size=S4)       # [B,  50, 152] -> [B, 197, 152]
        upsample4_2 = self.upsample(cur4, factor=4., size=S4)       # [B,  50, 152] -> [B, 785, 152]

        # x2下采样到x3和x4大小,x3下采样到x4大小
        downsample2_3 = self.downsample(cur2, factor=2., size=S2)   # [B, 785, 152] -> [B, 197, 152]
        downsample3_4 = self.downsample(cur3, factor=2., size=S3)   # [B, 197, 152] -> [B,  50, 152]
        downsample2_4 = self.downsample(cur2, factor=4., size=S2)   # [B, 785, 152] -> [B,  50, 152]

        # 将上下采样的数据相加
        cur2 = cur2 + upsample3_2 + upsample4_2     # [B, 785, 152] + [B, 785, 152] + [B, 785, 152] = [B, 785, 152]
        cur3 = cur3 + upsample4_3 + downsample2_3   # [B, 197, 152] + [B, 197, 152] + [B, 197, 152] = [B, 197, 152]
        cur4 = cur4 + downsample3_4 + downsample2_4 # [B,  50, 152] + [B,  50, 152] + [B,  50, 152] = [B,  50, 152]
...
class CoaT(nn.Module):
    ...
    def forward_features(self, x0):
        ...
        x1 = self.patch_embed1(x0)              # [B, 3, 224, 224] -> [B, 56*56, 64]                        [B, 56*56, 152]
        H1, W1 = self.patch_embed1.grid_size    # 56, 56
        x1 = insert_cls(x1, self.cls_token1)    # [B, 56*56, 64] cat [1, 1, 64] = [B, 3137, 64]             [B, 3137, 152]
        for blk in self.serial_blocks1:
            x1 = blk(x1, size=(H1, W1))         # [B, 3137, 64] -> [B, 3137, 64]                            [B, 3137, 152]
        # [B, 3137, 64] -> [B, 3136, 64] -> [B, 56, 56, 64] -> [B, 64, 56, 56]                              [B, 152, 56, 56]
        x1_nocls = remove_cls(x1).reshape(B, H1, W1, -1).permute(0, 3, 1, 2).contiguous()

        # Serial blocks 2.
        x2 = self.patch_embed2(x1_nocls)        # [B, 64, 56, 56] -> [B, 28*28, 128]                        [B, 28*28, 152]
        H2, W2 = self.patch_embed2.grid_size    # 28, 28
        x2 = insert_cls(x2, self.cls_token2)    # [B, 28*28, 128] cat [1, 1, 128] = [B, 785, 128]           [B, 785, 152]
        for blk in self.serial_blocks2:
            x2 = blk(x2, size=(H2, W2))         # [B, 785, 128] -> [B, 785, 128]                            [B, 785, 152]
        ...
        # Serial blocks 3.
        x3 = self.patch_embed3(x2_nocls)        # [B, 128, 28, 28] -> [B, 14*14, 256]                       [B, 14*14, 152]
        H3, W3 = self.patch_embed3.grid_size    # 14, 14
        x3 = insert_cls(x3, self.cls_token3)    # [B, 14*14, 256] cat [1, 1, 256] = [B, 197, 256]           [B, 197, 152]
        for blk in self.serial_blocks3:
            x3 = blk(x3, size=(H3, W3))         # [B, 197, 256] -> [B, 197, 256]                            [B, 197, 152]
        ...
        # Serial blocks 4.
        x4 = self.patch_embed4(x3_nocls)        # [B, 256, 14, 14] -> [B, 7*7, 320]                         [B, 7*7, 152]
        H4, W4 = self.patch_embed4.grid_size    # 7, 7
        x4 = insert_cls(x4, self.cls_token4)    # [B, 7*7, 320] -> [B, 50, 320]                             [B, 50, 152]
        for blk in self.serial_blocks4:
            x4 = blk(x4, size=(H4, W4))         # [B, 50, 320] -> [B, 50, 320]                              [B, 50, 152]
