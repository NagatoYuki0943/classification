...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + self.drop_path(self.mlp(self.norm2(x)))     # [B, 197, 384] -> [B, 197, 384]  mix channel
        return x
...
class ConvBlock(nn.Module):
    ...
    def forward(self, x, x_t=None, return_x_2=True):
        ...
        x = self.conv2(x) if x_t is None else self.conv2(x + x_t)   # 如果有x_t就相加 x_t是Transformer提取的输出,形状和conv2的输入输出相同
        x = self.bn2(x)
        if self.drop_block is not None:
            x = self.drop_block(x)
        x2 = self.act2(x)
...
class FCUDown(nn.Module):
    ...
    def forward(self, x, x_t):
        ...
        x = self.sample_pooling(x).flatten(2).transpose(1, 2)   # [B, 384, 56, 56] -> [B, 384, 16, 16] -> [B, 384, 196] -> [B, 196, 384]
        x = self.ln(x)
        x = self.act(x)

        x = torch.cat([x_t[:, 0][:, None, :], x], dim=1)    # [B, 1, 384] cat [B, 1, 384] = [B, 197, 384]
...
class FCUUp(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        x_r = self.act(self.bn(self.conv_project(x_r))) # [B, 384, 14, 14] -> [B, 16, 14, 14]

        return F.interpolate(x_r, size=(H * self.up_stride, W * self.up_stride))    # [B, 16, 14, 14] -> [B, 16, 64, 64]
...
class ConvTransBlock(nn.Module):
    ...
    def forward(self, x, x_t):
        ...
        #   残差部分
        #------------------#
        x, x2 = self.cnn_block(x)   # x: [B, 64, 56, 56] -> [B, 64, 56, 56] & [B, 16, 56, 56] x2是3x3Conv的返回值

        _, _, H, W = x2.shape       # 56, 56
        #------------------------------#
        #   融合残差块中3x3Conv的输出与Transformer的输入
        #------------------------------#
        x_st = self.squeeze_block(x2, x_t)  # [B, 16, 56, 56] with [B, 197, 384] -> [B, 197, 384]

        #------------------#
        #   Transformer部分
        #------------------#
        x_t = self.trans_block(x_st + x_t)  # [B, 197, 384] + [B, 197, 384] -> [B, 197, 384]    a TransformerBlock
        ...
        #   从融合的Transformer结果中取出卷积数据
        #------------------------------#
        x_t_r = self.expand_block(x_t, H // self.dw_stride, W // self.dw_stride)    # [B, 197, 384] -> [B, 16, 56, 56]
        x = self.fusion_block(x, x_t_r, return_x_2=False)                           # [B, 64, 56, 56] with [B, 16, 56, 56] -> [B, 64, 56, 56]
...
class Conformer(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, num_classes=1000, base_channel=64, channel_ratio=4, num_med_block=0,
        ...
        #--------------------#
        self.conv1 = nn.Conv2d(in_chans, 64, kernel_size=7, stride=2, padding=3, bias=False)  # [B, 3, 224, 224] -> [B, 65, 112, 112]
        ...
        self.conv_1 = ConvBlock(inplanes=64, outplanes=stage_1_channel, res_conv=True, stride=1)         # [B, 64, 56, 56] -> [B, 64, 56, 56]
        self.trans_patch_conv = nn.Conv2d(64, embed_dim, kernel_size=trans_dw_stride, stride=trans_dw_stride, padding=0) # [B, 64, 56, 56] -> [B, 384, 14, 14] -> [B, 384, 196] -> [B, 196, 384]
        self.trans_1 = Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, # [B, 197, 384] -> [B, 197, 384]
    ...
    def forward(self, x):
        ...
        #   stem stage
        #-----------------------------#
        x_base = self.maxpool(self.act1(self.bn1(self.conv1(x))))   # [B, 3, 224, 224] -> [B, 64, 112, 112] -> [B, 64, 56, 56]
        ...
        #   1 stage transformer
        #-----------------------------#
        x_t = self.trans_patch_conv(x_base).flatten(2).transpose(1, 2)# [B, 64, 56, 56] -> [B, 384, 14, 14] -> [B, 384, 196] -> [B, 196, 384]
        # vit的分类头
        cls_tokens = self.cls_token.expand(B, -1, -1)               # [1, 1, 384] -> [B, 1, 384]
        x_t = torch.cat([cls_tokens, x_t], dim=1)                   # [B, 1, 384] cat [B, 196, 384] -> [B, 197, 384]
        ...
        #   返回卷积和Transformer的结果
        #-----------------------------#
        for i in range(2, self.fin_stage):                          # [B, 64, 56, 56] -> [B, 128, 28, 28] -> [B, 256, 14, 14] -> [B, 256, 7, 7]
            x, x_t = eval('self.conv_trans_' + str(i))(x, x_t)      # [B, 197, 384] -> [B, 197, 384]
        ...
        tran_cls = self.trans_cls_head(x_t[:, 0])   # [B, 197, 384] 取出 [B, 384] -> [B, num_classes]
