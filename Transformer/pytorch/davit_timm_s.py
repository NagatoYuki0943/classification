...
from timm.layers import DropPath, to_2tuple, trunc_normal_, LayerNorm2d, get_norm_layer, use_fused_attn
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        k = k * self.scale
        attention = k.transpose(-1, -2) @ v     # [B, h, c, N] * [B, h, N, c] = [B, h, c, c]    !!!使用了通道的注意力!!! edgenext也用了这个
        attention = attention.softmax(dim=-1)   # 行上做softmax

        x = (attention @ q.transpose(-1, -2))   # [B, h, c, c] @ [B, h, c, N] = [B, h, c, N]
...
class ChannelBlock(nn.Module):
    ...
    def forward(self, x: Tensor):
        B, C, H, W = x.shape
        # 相对位置编码
        x = self.cpe1(x).flatten(2).transpose(1, 2) # [B, C, H, W] -> [B, C, H*W] -> [B, H*W, C]

        cur = self.norm1(x)
        cur = self.attn(cur)            # [B, H*W, C] -> [B, H*W, C]
        x = x + self.drop_path1(cur)    # [B, H*W, C] + [B, H*W, C] = [B, H*W, C]

        x = self.cpe2(x.transpose(1, 2).view(B, C, H, W))   # [B, H*W, C] -> [B, C, H*W] -> [B, C, H, W] -> [B, C, H, W]

        if self.mlp is not None:
            x = x.flatten(2)                                    # [B, C, H, W] -> [B, C, H*W]
            x = x.transpose(1, 2)                               # [B, C, H*W] -> [B, H*W, C]
            x = x + self.drop_path2(self.mlp(self.norm2(x)))    # [B, H*W, C] -> [B, H*W, 4*C] -> [B, H*W, C]
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        qkv = qkv.reshape(B_, N, 3, self.num_heads, C // self.num_heads)# [B_, N, 3*C] -> [B_, N, 3, h, c]      C = h * c
        ...
        if self.fused_attn:
            ...
            q = q * self.scale
            attn = (q @ k.transpose(-2, -1))    # [B_, h, N, c] @ [B_, h, c, N] = [B_, h, N, N]
...
class SpatialBlock(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        # DWConv位置编码
        shortcut = self.cpe1(x).flatten(2).transpose(1, 2)  # [B, C, H, W] -> [B, C, H, W] -> [B, C, H*W] -> [B, H*W, C]
        ...
        _, Hp, Wp, _ = x.shape
        # window_size = (7, 7)
        x_windows = window_partition(x, self.window_size)                                   # [B, H, W, C] -> [B*nh*nw, h, w, C]
        x_windows = x_windows.view(-1, self.window_size[0] * self.window_size[1], C)        # [B*nh*nw, h, w, C] -> [B*nh*nw, h*w, C]

        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows)                                                 # [B*nh*nw, h*w, C] -> [B*nh*nw, h*w, C]

        # merge windows
        attn_windows = attn_windows.view(-1, self.window_size[0], self.window_size[1], C)   # [B*nh*nw, h*w, C] -> [B*nh*nw, h, w, C]
        x = window_reverse(attn_windows, self.window_size, Hp, Wp)                          # [B*nh*nw, h, w, C] -> [B, H, W, C]
        ...
        if self.mlp is not None:
            ...
            x = x + self.drop_path2(self.mlp(self.norm2(x)))# [B, H*W, C] -> [B, H*W, 4*C] -> [B, H*W, C]
            x = x.transpose(1, 2).view(B, C, H, W)          # [B, H*W, C] -> [B, C, H*W] -> [B, C, H, W]
...
class DaVitStage(nn.Module):
    def __init__(
        ...
        for block_idx in range(depth):  # depth重复一次顺序使用(spatial, channel)
            ...
            for attn_idx, attn_type in enumerate(attn_types): # 顺序使用(spatial, channel)
    ...
    def forward(self, x: Tensor):#           stage1没有下采样
        x = self.downsample(x)  # [B, 96, 56, 56] -> [B, 96, 56, 56] -> [1, 192, 28, 28] -> [1, 384, 14, 14] -> [1, 768, 7, 7]
