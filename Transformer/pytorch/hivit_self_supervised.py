# https://github.com/zhangxiaosong18/hivit/blob/master/self_supervised/models/models_hivit.py
...
class Attention(nn.Module):
    ...
    def forward(self, x, rpe_index=None, mask=None):
        ...
        qkv = qkv.reshape(B, N, 3, self.num_heads, C // self.num_heads) # [B, N, 3*C] -> [B, N, 3, h, c]    C = h * c
        qkv = qkv.permute(2, 0, 3, 1, 4)                                # [B, N, 3, h, c] -> [3, B, h, N, c]
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy      # [3, B, h, N, c] -> 3 * [B, h, N, c]

        q = q * self.scale
        attn = (q @ k.transpose(-2, -1))    # [B, h, N, c] @ [B, h, c, N] = [B, h, N, N]

        if rpe_index is not None: # 相对位置编码
            S = int(math.sqrt(rpe_index.size(-1)))  # S = N
            relative_position_bias = self.relative_position_bias_table[rpe_index].view(-1, S, S, self.num_heads) # [1, N, N, h]
            relative_position_bias = relative_position_bias.permute(0, 3, 1, 2).contiguous()    # [1, N, N, h] -> [1, h, N, N]
            attn = attn + relative_position_bias    # [B, h, N, N] + [1, h, N, N] = [B, h, N, N]
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        patches_resolution = (H // self.patch_size[0], W // self.patch_size[1]) # [14, 14]
        ...
        x = x.permute(0, 2, 4, 3, 5, 1) # [B, 128, 14, 4 , 14, 4] -> [B, 14, 14, 4, 4, 128]
...
class PatchMerge(nn.Module):
    ...
    def forward(self, x):           # [B, num_patches, inner_patches, inner_patches, C]
        x0 = x[..., 0::2, 0::2, :]  # [B, num_patches, inner_patches/2, inner_patches/2, C] 左上
        x1 = x[..., 1::2, 0::2, :]  # [B, num_patches, inner_patches/2, inner_patches/2, C] 左下
        x2 = x[..., 0::2, 1::2, :]  # [B, num_patches, inner_patches/2, inner_patches/2, C] 右上
        x3 = x[..., 1::2, 1::2, :]  # [B, num_patches, inner_patches/2, inner_patches/2, C] 右下

        x = torch.cat([x0, x1, x2, x3], dim=-1) # [B, num_patches, inner_patches/2, inner_patches/2, 512]
        x = self.norm(x)
        x = self.reduction(x)   # [B, num_patches, inner_patches/2, inner_patches/2, 4*C] -> [B, num_patches, inner_patches/2, inner_patches/2, 2*C]
        return x
...
class HiViT(nn.Module):
    ...
    def forward_features(self, x, ids_keep=None, mask=None, return_hifeat=False):
        B = x.shape[0]
        x = self.patch_embed(x) # [B, 3, 224, 224] -> [B, 196, 4, 4, 128]
        if ids_keep is not None:
            x = torch.gather(
                x, dim=1, index=ids_keep[:, :, None, None, None].expand(-1, -1, *x.shape[2:])
            )
        hifeat = []
        for blk in self.blocks[:-self.num_main_blocks]: # [B, 196, 4, 4, 128] -> [B, 196, 2, 2, 256] -> [B, 196, 1, 1, 512]
            if isinstance(blk, PatchMerge):
                hifeat.append(x)
            x = checkpoint.checkpoint(blk, x) if self.use_checkpoint else blk(x)

        x = x[..., 0, 0, :] # [B, 196, 1, 1, 512] -> [B, 196, 512]
        if self.ape: #  绝对位置编码
            pos_embed = self.absolute_pos_embed # [1, 196, 512]
            if ids_keep is not None: # None
                pos_embed = torch.gather(
                    pos_embed.expand(B, -1, -1),
                    dim=1,
                    index=ids_keep[:, :, None].expand(-1, -1, pos_embed.shape[2]),
                )
            x += pos_embed # [B, 196, 512] + [1, 196, 512] = [B, 196, 512]
        x = self.pos_drop(x)
        ...
        if self.rpe:
            if ids_keep is not None: # None
                ...
                rpe_index = self.relative_position_index.view(-1)   # [196*196]

        for blk in self.blocks[-self.num_main_blocks:]:             # [B, 196, 512] -> [B, 196, 512]
            x = checkpoint.checkpoint(blk, x, rpe_index, mask) if self.use_checkpoint else blk(x, rpe_index, mask)

        return x if not return_hifeat else hifeat + [x]

    def forward(self, x):
        x = self.forward_features(x)    # [B, 3, 224, 224] -> [B, 196, 512]
        x = x.mean(dim=1)               # [B, 196, 512] -> [B, 512]
        x = self.fc_norm(x)
        x = self.head(x)                # [B, 512] -> [B, num_classes]
        return x
...
if __name__ == "__main__":
    ...
    model = hivit_base(pretrained=False, num_classes=5).to(device)
