...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.flatten:
            x = x.flatten(2).transpose(1, 2)  # [B, 768, 14, 14] -> [B, 768, 196] -> [B, 196, 768]
...
class MixerBlock(nn.Module):
    ...
    def forward(self, x):
        residual = x
        x = self.norm1(x).transpose(1, 2)       # [B, 196, 768] -> [B, 768, 196]    mix position
        x = self.mlp_tokens(x).transpose(1, 2)  # [B, 768, 196] -> [B, 768, 196] -> [B, 196, 768]
        x = residual + self.drop_path(x)

        residual = x
        x = self.mlp_channels(self.norm2(x))    # [B, 196, 768] -> [B, 196, 768]    mix channel
        x = residual + self.drop_path(x)
        return x
...
class Affine(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.alpha = nn.Parameter(torch.ones((1, 1, dim)))  # [1, 1, 384] 384是最后的channel
...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        residual = x
        x = self.norm1(x).transpose(1, 2)                       # [B, 196, 384] -> [B, 384, 196]                            mix position
        x = self.ls1 * self.linear_tokens(x).transpose(1, 2)    # [B, 384, 196] -> [B, 384, 196] -> [B, 196, 384] -> [384] * [B, 196, 384] -> [B, 196, 384]
        x = residual + self.drop_path(x)

        residual = x
        x = self.ls2 * self.mlp_channels(self.norm2(x))         # [B, 196, 384] -> [384] * [B, 196, 384] -> [B, 196, 384]   mix channel
        x = residual + self.drop_path(x)
        return x
...
class SpatialGatingUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        v = self.norm(v)
        v = self.proj(v.transpose(-1, -2))  # [B, P, C/2] -> [B, C/2, P] -> [B, C/2, P] 对 seq 维度做投影
        return u * v.transpose(-1, -2)      # [B, P, C/2] * ([B, C/2, P] -> [B, P, C/2]) = [B, P, C/2]  最终通道减半
...
class MlpMixer(nn.Module):
    ...
    def forward_head(self, x, pre_logits: bool = False):
        if self.global_pool == 'avg':
            x = x.mean(dim=1)   # [B, 196, 384] -> [B, 384]    这里在dim=1上求均值,196=14*14,代表了高宽为14的图形,所以相当于求全图的均值
        x = self.head_drop(x)
