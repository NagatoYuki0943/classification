...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = self.qkv(
            x
        )  # [B, T, 196, C] -> [B, T, 196, C*3]        C = h * 32  per head has 32 channels
        qkv = qkv.reshape(
            B, T, N, 3, self.num_heads, C // self.num_heads
        )  # [B, T, 196, 96*3] -> [B, T, 196, 3, h, 32]
        qkv = qkv.permute(
            3, 0, 4, 1, 2, 5
        )  # [B, T, 196, 3, h, 32] -> [3, B, h, T, 196, 32]
        q, k, v = qkv.unbind(
            0
        )  # make torchscript happy                     [3, B, h, T, 196, 32] -> [B, h, T, 196, 32] * 3
        ...
        if self.fused_attn:
            ...
            attn = (
                q @ k.transpose(-2, -1)
            )  # (B, H, T, N, N)    [B, h, T, 196, 32] @ [B, h, T, 32, 196] = [B, h, T, 196, 196]
            attn = attn.softmax(dim=-1)  # 取每一列,在行上做softmax
            attn = self.attn_drop(attn)
            x = (
                attn @ v
            )  # [B, h, T, 196, 196] @ [B, h, T, 196, 32] = [B, h, T, 196, 32]
...
class ConvPool(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        _assert(
            x.shape[-2] % 2 == 0, "BlockAggregation requires even input spatial dims"
        )
        _assert(
            x.shape[-1] % 2 == 0, "BlockAggregation requires even input spatial dims"
        )
        x = self.conv(x)
...
def blockify(x, block_size: int):
    ...
    x = x.reshape(
        B, grid_height, block_size, grid_width, block_size, C
    )  # [B, 56, 56, 96] -> [B, 4, 14, 4, 14, 96]
    x = x.transpose(2, 3).reshape(
        B, grid_height * grid_width, -1, C
    )  # [B, 4, 14, 4, 14, 96] -> [B, 4, 4, 14, 14, 96] -> [B, 16, 196, 96]
...
def deblockify(x, block_size: int):
    ...
    x = x.transpose(2, 3).reshape(
        B, height, width, C
    )  # [B, 16, 196, 96] -> [B, 16, 96, 196] -> [B, 56, 56, 96]
    return x  # (B, H, W, C)
...
class NestLevel(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pool(
            x
        )  # [B, 96, 56, 56] -> [B, 96, 56, 56] -> [B, 192, 28, 28] -> [B, 384, 14, 14]
        x = x.permute(
            0, 2, 3, 1
        )  #                    [B, 56, 56, 96]    [B, 28, 28, 192]    [B, 14, 14, 384]
        x = blockify(
            x, self.block_size
        )  #                    [B, 16, 196, 96]   [B, 4, 196, 192]    [B, 1, 196, 384]
        x = (
            x + self.pos_embed
        )  #                    [B, 16, 196, 96]   [B, 4, 196, 192]    [B, 1, 196, 384]
        ...
        return x.permute(
            0, 3, 1, 2
        )  # (B, C, H', W')           [B, 96, 56, 56]    [B, 192, 28, 28]    [B, 384, 14, 14]
...
class Nest(nn.Module):
    ...
    def __init__(
        ...
        for param_name in ["embed_dims", "num_heads", "depths"]:
            ...
            if isinstance(param_value, collections.abc.Sequence):
                assert (
                    len(param_value) == num_levels
                ), f"Require `len({param_name}) == num_levels`"
        ...
        assert (
            (img_size // patch_size) % math.sqrt(self.num_blocks[0]) == 0
        ...
        for i in range(len(self.num_blocks)):
            ...
            self.feature_info += [
                dict(num_chs=dim, reduction=curr_stride, module=f"levels.{i}")
            ]
            prev_dim = dim
            curr_stride *= 2
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x.permute(0, 2, 3, 1)).permute(
            0, 3, 1, 2
        )  # [B, 384, 14, 14] -> [B, 14, 14, 384] -> [B, 384, 14, 14]
        return x
...
def resize_pos_embed(posemb, posemb_new):
    ...
    posemb = F.interpolate(
        posemb, size=[size_new, size_new], mode="bicubic", align_corners=False
    )
...
def nest_small(pretrained=False, **kwargs) -> Nest:
    """Nest-S @ 224x224"""
    model_kwargs = dict(
        embed_dims=(96, 192, 384), num_heads=(3, 6, 12), depths=(2, 2, 20), **kwargs
    )
...
def nest_tiny(pretrained=False, **kwargs) -> Nest:
    """Nest-T @ 224x224"""
    model_kwargs = dict(
        embed_dims=(96, 192, 384), num_heads=(3, 6, 12), depths=(2, 2, 8), **kwargs
    )
