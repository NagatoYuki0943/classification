...
from timm.layers import lecun_normal_, DropPath, ClassifierHead, to_2tuple, _assert
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        # k = s = 7
        x = self.proj(x)  # [B, 3, 224, 224] -> [B, 192, 32, 32]
        if self.flatten:
            x = x.flatten(2).transpose(1, 2)  # NCHW -> NLC
        elif self.output_fmt != Format.NCHW:
            x = nchw_to(x, self.output_fmt)  # [B, 192, 32, 32] -> [B, 32, 32, 192]
        x = self.norm(x)
        return x
...
class Mlp(nn.Module):
    ...
    def forward(self, x):  # mix channel
        x = self.fc1(x)  # [B, H, W, C] -> [B, H, W, n*C]
        x = self.act(x)
        x = self.drop1(x)
        x = self.norm(x)
        x = self.fc2(x)  # [B, H, W, n*C] -> [B, H, W, C]
        x = self.drop2(x)
        return x
...
class RNN2dBase(nn.Module):
    ...
    def forward(self, x):
        B, H, W, C = x.shape  # [1, 32, 32, 192]

        # vertical和horizontal是并行的
        if self.with_vertical:
            v = x.permute(0, 2, 1, 3)  # [B, H, W, C] -> [B, W, H, C]
            v = v.reshape(-1, H, C)  # [B, W, H, C] -> [B*W, H, C]
            v, _ = self.rnn_v(v)  # [B*W, H, C] -> [B*W, H, newC]     在H方向做rnn
            v = v.reshape(B, W, H, -1)  # [B*W, H, newC] -> [B, W, H, newC]
            v = v.permute(0, 2, 1, 3)  # [B, W, H, newC] -> [B, H, W, newC]
        else:
            v = None

        if self.with_horizontal:
            h = x.reshape(-1, W, C)  # [B, H, W, C] -> [B*H, W, C]
            h, _ = self.rnn_h(h)  # [B*H, W, C] -> [B*H, W, newC]     在W方向做rnn
            h = h.reshape(B, H, W, -1)  # [B*H, W, newC] -> [B, H, W, newC]
        else:
            h = None

        # 将vertical和horizontal结果拼起来
        if v is not None and h is not None:
            if self.union == "cat":
                x = torch.cat(
                    [v, h], dim=-1
                )  # [B, H, W, newC] cat [B, H, W, newC] = [B, H, W, 2*newC]
            else:
                x = v + h
        elif v is not None:
            x = v
        ...
        if self.fc is not None:
            x = self.fc(x)  # [B, H, W, 2*newC] -> [B, H, W, 2*newC]    2*newC = C
...
class Sequencer2dBlock(nn.Module):
    ...
    def forward(self, x):
        x = x + self.drop_path(
            self.rnn_tokens(self.norm1(x))
        )  # [B, H, W, C] -> [B, H, W, C]
        x = x + self.drop_path(
            self.mlp_channels(self.norm2(x))
        )  # [B, H, W, C] -> [B, H, W, C]
        return x
...
class Sequencer2dStage(nn.Module):
    ...
    def forward(self, x):  # 第一次没有下采样
        x = self.downsample(
            x
        )  # [B, 32, 32, 192] -> [B, 32, 32, 192] -> [B, 16, 16, 384] -> [B, 16, 16, 384]
        x = self.blocks(x)  # 形状不变
        return x
...
class Sequencer2d(nn.Module):
    def __init__(
        ...
        for i, _ in enumerate(embed_dims):
            ...
            self.feature_info += [
                dict(num_chs=prev_dim, reduction=reductions[i], module=f"stages.{i}")
            ]
    ...
    def forward_features(self, x):
        x = self.stem(x)  # [B, 3, 224, 224] -> [B, 32, 32, 192]  PatchEmbed k = s = 7
        x = self.stages(x)  # [B, 32, 32, 192] -> [B, 16, 16, 384]
        x = self.norm(x)
        return x
    ...
    def forward(self, x):
        x = self.forward_features(x)  # [B, 3, 224, 224] -> [B, 16, 16, 384]
        x = self.forward_head(x)  # [B, 16, 16, 384] -> [B, num_classes]
        return x
...
def checkpoint_filter_fn(state_dict, model):
    ...
    for k, v in state_dict.items():
        k = re.sub(
            r"blocks.([0-9]+).([0-9]+).down",
            lambda x: f"stages.{int(x.group(1)) + 1}.downsample.down",
            k,
        )
...
def sequencer2d_s(pretrained=False, **kwargs) -> Sequencer2d:
    ...
    model = _create_sequencer2d(
        "sequencer2d_s", pretrained=pretrained, **dict(model_args, **kwargs)
    )
    return model
...
def sequencer2d_m(pretrained=False, **kwargs) -> Sequencer2d:
    ...
    model = _create_sequencer2d(
        "sequencer2d_m", pretrained=pretrained, **dict(model_args, **kwargs)
    )
    return model
...
def sequencer2d_l(pretrained=False, **kwargs) -> Sequencer2d:
    ...
    model = _create_sequencer2d(
        "sequencer2d_l", pretrained=pretrained, **dict(model_args, **kwargs)
    )
    return model
...
if __name__ == "__main__":
    ...
    model = sequencer2d_s(pretrained=False, num_classes=5).to(device)
