...
class RNN2dBase(nn.Module):
    ...
    def forward(self, x):
        ...
        if v is not None and h is not None:
            if self.union == "cat":
                x = torch.cat([v, h], dim=-1)   # [B, H, W, newC] cat [B, H, W, newC] = [B, H, W, 2*newC]
            else:
                x = v + h
        elif v is not None:
            x = v
...
class Sequencer2dBlock(nn.Module):
    ...
    def forward(self, x):
        x = x + self.drop_path(self.rnn_tokens(self.norm1(x)))      # [B, H, W, C] -> [B, H, W, C]
        x = x + self.drop_path(self.mlp_channels(self.norm2(x)))    # [B, H, W, C] -> [B, H, W, C]
        return x
...
class Sequencer2dStage(nn.Module):
    ...
    def forward(self, x):                  # 第一次没有下采样
        x = self.downsample(x)  # [B, 32, 32, 192] -> [B, 32, 32, 192] -> [B, 16, 16, 384] -> [B, 16, 16, 384]
