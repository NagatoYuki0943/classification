    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        if mask is not None:
            ...
            attn = attn.view(B_ // nW, nW, self.num_heads, N, N) + mask.unsqueeze(
                ...
                0
            )  # mask相同区域使用0表示,不同区域使用-100表示,通过softmax之后就很小了
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, attn_mask):
        ...
        )  # [B, 56, 56, 96] -> [b*64, 7, 7, 96] 64是nW,64个窗口; 7是Mh和Mw,每个小窗口的宽高
        ...
        attn_windows = attn_windows.view(
            -1, self.window_size, self.window_size, C
        )  # [b*64, 49, 96] -> [b*64, 7, 7, 96]
        shifted_x = window_reverse(
            attn_windows, self.window_size, Hp, Wp
        )  # [b*64, 7, 7, 96] -> [B, 56, 56, 96]
...
class BasicLayer(nn.Module):
        ...
        self.blocks = nn.ModuleList(
            [
                SwinTransformerBlock(
                    ...
                    shift_size=0
                    if (i % 2 == 0)
                    else self.shift_size,  # W-SMA or SM-WSA   0,2...为W-SMA
                    mlp_ratio=mlp_ratio,
                    qkv_bias=qkv_bias,
                    drop=drop,
                    attn_drop=attn_drop,
    ...
    def create_mask(self, x, H, W):
        ...
        # 分块
        h_slices = (
            slice(0, -self.window_size),  # 假设9x9,分成3份,shift=1,取0~5
            slice(-self.window_size, -self.shift_size),  # 假设9x9,分成3份,shift=1,取6~7
            slice(-self.shift_size, None),
        )  # 假设9x9,分成3份,shift=1,取8    下面的w相同
        w_slices = (
        ...
        )  # [B*num_windows, Mh, Mw, 1] = [所有窗口,窗口高,窗口宽,通道]
        mask_windows = mask_windows.view(
            -1, self.window_size * self.window_size
        )  # [B*num_windows, Mh*Mw] 每个window都按照行展平
        attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(
            2
        )  # [B*num_windows, 1, Mh*Mw] - [B*num_windows, Mh*Mw, 1]
        # [nW, Mh*Mw, Mh*Mw]
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i_layer in range(self.num_layers):
            ...
            layers = BasicLayer(
                dim=int(embed_dim * 2**i_layer),  # 通道数
                ...
                else None,  # 下采样方式,判断是前三个有下采样,最后没有
