...
class TokenAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = self.qkv(
            x
        )  # [B, 56*56, 147] -> [1, 56*56, 192]                    [B, 784, 576] -> [B, 784, 192]
        qkv = qkv.reshape(
            B, N, 3, self.num_heads, self.in_dim
        )  # [B, 56*56, 192] -> [B, 56*56, 3, 1, 64]               [B, 784, 192] -> [B, 784, 3, 1, 64]
        qkv = qkv.permute(
            2, 0, 3, 1, 4
        )  # [B, 56*56, 3, 1, 64] -> [3, B, 1, 56*56, 64]          [B, 784, 3, 1, 64] -> [3, B, 1, 784, 64]
        q, k, v = (
            qkv[0],
            qkv[1],
            qkv[2],
        )  # [3, B, 1, 56*56, 64] -> [B, 1, 56*56, 64] * 3         [3, B, 1, 784, 64] -> [B, 1, 784, 64] * 3

        attn = (
            (q * self.scale) @ k.transpose(-2, -1)
        )  # [B, 1, 56*56, 64] @ [B, 1, 64, 56*56] = [B, 1, 56*56, 56*56]  [B, 1, 784, 64] @ [B, 1, 64, 784] = [B, 1, 784, 784]
        attn = attn.softmax(dim=-1)  # 取每一列,在行上做softmax
        attn = self.attn_drop(attn)

        x = (
            attn @ v
        )  # [B, 1, 56*56, 56*56] @ [B, 1, 56*56, 64] = [B, 1, 56*56, 64]  [B, 1, 784, 784] @ [B, 1, 784, 64] = [B, 1, 784, 64]
        x = x.transpose(
            1, 2
        )  # [B, 1, 56*56, 64] -> [B, 56*56, 1, 64]                        [B, 1, 784, 64] -> [B, 784, 1, 64]
        x = x.reshape(
            B, N, self.in_dim
        )  # [B, 56*56, 1, 64] -> [B, 56*56, 64]                           [B, 784, 1, 64] -> [B, 784, 64]
        x = self.proj(
            x
        )  # [B, 56*56, 64] -> [B, 56*56, 64]                              [B, 784, 64] -> [B, 784, 64]
        x = self.proj_drop(x)
...
class Token_transformer(nn.Module):
    ...
    def forward(self, x):
        x = self.attn(
            self.norm1(x)
        )  # [B, 56*56, 147] -> [B, 56*56, 64]     [B, 784, 576] -> [B, 784, 64]
        x = x + self.drop_path(
...
class Token_performer(nn.Module):
    ...
    def single_attn(self, x):
        kqv = self.kqv(
            x
        )  # [B, 56*56, 147] -> [B, 56*56, 192]        [B, 784, 576] -> [B, 784, 192]
        k, q, v = torch.split(
            kqv, self.emb, dim=-1
        )  # [B, 56*56, 192] -> [B, 56*56, 64] * 3     [B, 784, 192] -> [B, 784, 64] * 3
        kp, qp = (
            self.prm_exp(k),
            self.prm_exp(q),
        )  # (B, T, m), (B, T, m) [B, 56*56, 32]       [B, 784, 32]
        D = torch.einsum("bti,bi->bt", qp, kp.sum(dim=1)).unsqueeze(
            dim=2
        )  # (B, T, m) * (B, m) -> (B, T, 1)       [B, 56*56, 1]   [B, 784, 1]
        kptv = torch.einsum(
            "bin,bim->bnm", v.float(), kp
        )  # (B, emb, m)                                           [B, 64, 32]     [B, 64, 32]
        y = torch.einsum("bti,bni->btn", qp, kptv) / (
            D.repeat(1, 1, self.emb) + self.epsilon
        )  # (B, T, emb)/Diag  [B, 56*56, 64]  [B, 784, 64]
        # skip connection
        y = (
            v + self.dp(self.proj(y))
        )  # same as token_transformer in T2T layer, use v as skip connection   [B, 56*56, 64]  [B, 784, 64]
...
class T2T_module(nn.Module):
    ...
    def forward(self, x):
        # step0: soft split
        x = self.soft_split0(x).transpose(
            1, 2
        )  # [B, 3, 224, 224] -> [B, 147, 56*56] -> [B, 56*56, 147]
        ...
        x = self.soft_split1(x).transpose(
            1, 2
        )  # [B, 64, 56, 56] -> [B, 576, 28*28] -> [B, 784, 576]
        ...
        x = self.soft_split2(x).transpose(
            1, 2
        )  # [B, 64, 28, 28] -> [B, 576, 196] -> [B, 196, 576]
...
class T2T_ViT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = torch.cat(
            (cls_tokens, x), dim=1
        )  # [1, 1, 384] cat [B, 196, 384] = [B, 197, 384]
