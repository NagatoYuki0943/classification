...
class PixelEmbed(nn.Module):
    ...
    def forward(self, x, pixel_pos):
        ...
        temp0 = x.reshape(B, self.in_dim, H//self.stride//new_patch_size, new_patch_size,
                        W//self.stride//new_patch_size, new_patch_size)             # [B, 24, 56, 56] -> [B, 24, 14, 4, 14, 4]
        temp0 = temp0.permute(0, 1, 3, 5, 2, 4)                                     # [B, 24, 14, 4, 14, 4] -> [B, 24, 4, 4, 14, 14]
        temp0 = temp0.reshape(B, self.in_dim*new_patch_size*new_patch_size, -1)     # [B, 24, 4, 4, 14, 14] -> [B, 384, 196]
        # [56] -> [4, 14]
        temp1 = x.reshape(B, self.in_dim, new_patch_size, H//self.stride//new_patch_size,
                         new_patch_size, W//self.stride//new_patch_size)            # [B, 24, 56, 56] -> [B, 24, 4, 14, 4, 14]
        temp1 = temp1.permute(0, 1, 2, 4, 3, 5)                                     # [B, 24, 4, 14, 4, 14] -> [B, 24, 4, 4, 14, 14]
        temp1 = temp1.reshape(B, self.in_dim*new_patch_size*new_patch_size, -1)     # [B, 24, 4, 4, 14, 14] -> [B, 384, 196]
        # print(temp0.eq(temp1).sum().item())  # 75264 = 1 * 384 * 196
        #-----------------------------------------------------------------------------------------------------------------------#

        x = self.unfold(x)                                      # [B, 24, 56, 56] -> [B, 384, 196]  56/4=14 14*14=196 24*4*4=384
        x = x.transpose(1, 2)                                   # [B, 384, 196] -> [B, 196, 384]
                                                                # [B, 196, 384] -> [B*196, 24, 4, 4]
        x = x.reshape(B * self.num_patches, self.in_dim, self.new_patch_size[0], self.new_patch_size[1])
        x = x + pixel_pos                                       # [B*196, 24, 4, 4] + [1, 24,  4,  4] = [B*196, 24, 4, 4]
        x = x.reshape(B * self.num_patches, self.in_dim, -1)    # [B*196, 24, 4, 4] -> [B*196, 24, 16]
...
def test_reshape(shape: list[int] = [1, 24, 56, 56], new_patch_size: int = 4):
    ...
    # [56] -> [14, 4]
    y1 = x.reshape(B, C, H//new_patch_size, new_patch_size, W//new_patch_size, new_patch_size)  # [B, 24, 56, 56] -> [B, 24, 14, 4, 14, 4]
    y1 = y1.permute(0, 1, 3, 5, 2, 4)                                                           # [B, 24, 14, 4, 14, 4] -> [B, 24, 4, 4, 14, 14]
    y1 = y1.reshape(B, C*new_patch_size*new_patch_size, -1)                                     # [B, 24, 4, 4, 14, 14] -> [B, 384, 196]

    # [56] -> [4, 14]
    y2 = x.reshape(B, C, new_patch_size, H//new_patch_size, new_patch_size, W//new_patch_size)  # [B, 24, 56, 56] -> [B, 24, 4, 14, 4, 14]
    y2 = y2.permute(0, 1, 2, 4, 3, 5)                                                           # [B, 24, 4, 14, 4, 14] -> [B, 24, 4, 4, 14, 14]
    y2 = y2.reshape(B, C*new_patch_size*new_patch_size, -1)                                     # [B, 24, 4, 4, 14, 14] -> [B, 384, 196]

    y3 = nn.Unfold(kernel_size=new_patch_size, stride=new_patch_size)(x)                        # [B, 24, 56, 56] -> [B, 384, 196]
...
class Mlp(nn.Module):
    ...
    def forward(self, x):   # mlp_tokens[B, channel, position]      mlp_channels[B, position, channel]
        x = self.fc1(x)     # [B, 768, 196] -> [B, 768, 384]        [B, 196, 768] -> [B, 196, 3072]
        x = self.act(x)
        x = self.drop1(x)
        x = self.norm(x)
        x = self.fc2(x)     # [B, 768, 384] -> [B, 768, 196]        [B, 196, 3072] -> [B, 196, 768]
        x = self.drop2(x)
        return x
...
class Attention(nn.Module):
    ...
    def forward(self, x):                                                           # pixel_embed               patch_embed
        ...
        qk = qk.reshape(B, N, 2, self.num_heads, self.head_dim)                     # [B*196, 16, 2, 4, 6]      [B, 197, 2, 6, 64]
        qk = qk.permute(2, 0, 3, 1, 4)                                              # [2, b*196, 4, 16, 6]      [2, b, 6, 197, 64]
        q, k = qk.unbind(0)   # make torchscript happy (cannot use tensor as tuple) # [B*196, 4, 16, 6] * 2     [B, 6, 197, 64] * 2
        v = self.v(x).reshape(B, N, self.num_heads, -1).permute(0, 2, 1, 3)         # [B*196, 4, 16, 6]         [B, 6, 197, 64]
        ...
        x = attn @ v                            # [B*196, 4, 16, 16] @ [B*196, 4, 16, 6] = [B*196, 4, 16, 6]    [B, 6, 197, 197] @ [B, 6, 197, 64] = [B, 6, 197, 64]
        x = x.transpose(1, 2).reshape(B, N, -1) # [B*196, 4, 16, 6] -> [B*196, 16, 4, 6] -> [B*196, 16, 24]     [B, 6, 197, 64] -> [B, 197, 6, 64] -> [B, 197, 384]
        x = self.proj(x)                        # [B*196, 16, 24] -> [B*196, 16, 24]                            [B, 197, 384] -> [B, 197, 384]
        x = self.proj_drop(x)
        return x
...
class Block(nn.Module):
    ...
    def forward(self, pixel_embed, patch_embed):
        ...
        """
        # inner
        pixel_embed = pixel_embed + self.drop_path(self.attn_in(self.norm_in(pixel_embed)))     # [B*196, 16, 24] -> [B*196, 16, 24]
        pixel_embed = pixel_embed + self.drop_path(self.mlp_in(self.norm_mlp_in(pixel_embed)))  # [B*196, 16, 24] -> [B*196, 16, 24]

        # outer
        # patch_embed 全局注意力和mlp
        # 先将pixel_embed reshape成为和patch_embed相同的形状,拼接起来融合特征再进行全局注意力和mlp
        B, N, C = patch_embed.size()    # [B, 197, 384]
        temp_pixel_embed = self.norm1_proj(pixel_embed).reshape(B, N - 1, -1)                   # [B*196, 16, 24] -> [B, 196, 384]
        ...
        patch_embed = torch.cat(                                                                # [B, 1, 384] cat ([B, 196, 384] + [B, 196, 384]) = [B, 197, 384]
            [patch_embed[:, 0:1], patch_embed[:, 1:] + temp_pixel_embed],
            dim=1)  # 取出分类层[B, 1, 384], 后面的位置[1, 196, 384]和pixel_embed相加然后再拼接到一起
        patch_embed = patch_embed + self.drop_path(self.attn_out(self.norm_out(patch_embed)))   # [B, 197, 384] -> [B, 197, 384]
        patch_embed = patch_embed + self.drop_path(self.mlp(self.norm_mlp(patch_embed)))        # [B, 197, 384] -> [B, 197, 384]
...
class TNT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        patch_embed = self.norm2_proj(self.proj(patch_embed))                           # [B, 196, 384] -> [B, 196, 384]
        patch_embed = torch.cat((self.cls_token.expand(B, -1, -1), patch_embed), dim=1) # [B, 1, 384] cat [B, 196, 384] -> [B, 197, 384]
        patch_embed = patch_embed + self.patch_pos                                      # [B, 197, 384] + [B, 197, 384] = [B, 197, 384]
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            for blk in self.blocks:
                pixel_embed, patch_embed = blk(pixel_embed, patch_embed)        # [B*196, 16, 24] & [B, 197, 384] -> [B*196, 16, 24] & [B, 197, 384]
    ...
    def forward_head(self, x, pre_logits: bool = False):
        if self.global_pool:
            x = x[:, 1:].mean(dim=1) if self.global_pool == 'avg' else x[:, 0]  # 不是avg,所以是 x[:, 0] = [B, 384]
        x = self.head_drop(x)
