...
class Attention(nn.Module):
    def __init__(
        ...
        attn_drop_ratio=0.0,  # attention dropout参数
        proj_drop_ratio=0.0,  # 最后的dropout参数
    ):
        super().__init__()
...
class Block(nn.Module):
    def __init__(
        ...
        mlp_ratio=4.0,  # Mlp第一个linear的全连接层扩展倍数
        qkv_bias=False,  # 是否q,v,k的bias
        qk_scale=None,  # 是否使用qk_scale, \sqrt d
        drop_ratio=0.0,  # Mlp最后的dropout比例
        attn_drop_ratio=0.0,  # Attention中的attn的dropout比例
        drop_path_ratio=0.0,  # 下面使用DropPath的比例
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        drop_ratio=0.0,  # Mlp最后的dropout
        attn_drop_ratio=0.0,  # Attention中的attn的dropout
        drop_path_ratio=0.0,  # Block中使用DropPath
        ...
        self.num_features = self.embed_dim = (
            embed_dim  # num_features for consistency with other models 768
        )
        # -----------------------------------------------------------#
        #   类别token,维度为1
        # -----------------------------------------------------------#
