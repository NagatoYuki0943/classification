...
class OutlookAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        h, w = (
            math.ceil(H / self.stride),
            math.ceil(W / self.stride),
        )  # 28 / 2 = [14, 14]
        ...
        v = v.permute(
            0, 1, 4, 3, 2
        )  # B,H,N,kxk,C/H                     [B, 6, 32, 9, 196] -> [B, 6, 196, 9, 32]

        attn = self.pool(
            x.permute(0, 3, 1, 2)
        ).permute(
            0, 2, 3, 1
        )  # [B, 28, 28, 192] -> [B, 192, 28, 28] -> [B, 192, 14, 14] -> [B, 14, 14, 192]
        ...
        attn = attn.permute(
            0, 2, 1, 3, 4
        )  # B,H,N,kxk,kxk               [B, 196, 6, 9, 9] -> [B, 6, 196, 9, 9]
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = qkv.reshape(
            B, H * W, 3, self.num_heads, C // self.num_heads
        )  # [B, H, W, 3*C] -> [B, H*W, 3, h, c]  C = h * c
        qkv = qkv.permute(2, 0, 3, 1, 4)  # [B, H*W, 3, h, c] -> [3, B, h, H*W, c]
        q, k, v = qkv.unbind(0)  # [3, B, h, H*W, c] -> 3 * [B, h, H*W, c]

        attn = (
            q @ k.transpose(-2, -1)
        ) * self.scale  # [B, h, H*W, c] @ [B, h, c, H*W] = [B, h, H*W, H*W]
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
...
class ClassBlock(nn.Module):
    ...
    def forward(self, x):
        cls_embed = x[
            :, :1
        ]  # [:1] 会保留维度 [0] 不会保留维度 [B, 197, 384] get [B, 1, 384]
        cls_embed = cls_embed + self.drop_path(
            self.attn(self.norm1(x))
        )  # [B, 1, 384] + [B, 1, 384] = [B, 1, 384]
        cls_embed = cls_embed + self.drop_path(
            self.mlp(self.norm2(cls_embed))
        )  # [B, 1, 384] + [B, 1, 384] = [B, 1, 384]
        return torch.cat(
            [cls_embed, x[:, 1:]], dim=1
...
class VOLO(nn.Module):
    ...
    def forward_cls(self, x):
        ...
        x = torch.cat(
            [cls_tokens, x], dim=1
        )  # [B, 196, 384] cat [B, 1, 384] = [B, 197, 384]
    ...
    def forward_features(self, x):
        x = self.patch_embed(x).permute(
            0, 2, 3, 1
        )  # [B, 3, 224, 224] -> [B, 192, 28, 28] -> [B, 28, 28, 192]
    ...
    def forward_head(self, x, pre_logits: bool = False):
        ...
        if self.aux_head is not None:
            ...
            aux = self.aux_head(
                x[:, 1:]
            )  # [B, 197, 384] get [B, 196, 384] -> [B, 196, num_classes]
            out = (
                out + 0.5 * aux.max(1)[0]
            )  # [B, num_classes] + 0.5 * ([B, 196, num_classes] -> [B, num_classes]) = [B, num_classes]
        return out
