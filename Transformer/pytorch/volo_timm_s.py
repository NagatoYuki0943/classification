...
4个stage:
    ...
    stage2,3,4:
        patch_embedding 2*2
        self attention
添加cls_token之后还有class attention
------------------------------------
"""
...
class OutlookAttention(nn.Module):
    ...
    def forward(self, x):
        B, H, W, C = x.shape    # [B, 28, 28, 192]

        v = self.v(x).permute(0, 3, 1, 2)                               # [B, 28, 28, 192] -> [B, 192, 28, 28]

        h, w = math.ceil(H / self.stride), math.ceil(W / self.stride)   # 28 / 2 = [14, 14]
        v = self.unfold(v)                                              # [B, 192, 28, 28] -> [B, 1728, 196]
        v = v.reshape(                                                  # [B, 1728, 196] -> [B, 6, 32, 9, 196]
            B, self.num_heads, C // self.num_heads,
            self.kernel_size * self.kernel_size, h * w
        )
        v = v.permute(0, 1, 4, 3, 2)  # B,H,N,kxk,C/H                     [B, 6, 32, 9, 196] -> [B, 6, 196, 9, 32]

        attn = self.pool(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)     # [B, 28, 28, 192] -> [B, 192, 28, 28] -> [B, 192, 14, 14] -> [B, 14, 14, 192]
        attn = self.attn(attn)                                          # [B, 14, 14, 192] -> [B, 14, 14, 486]
        attn = attn.reshape(                                            # [B, 14, 14, 486] -> [B, 196, 6, 9, 9]
            B, h * w, self.num_heads, self.kernel_size * self.kernel_size,
            self.kernel_size * self.kernel_size
        )
        attn = attn.permute(0, 2, 1, 3, 4)  # B,H,N,kxk,kxk               [B, 196, 6, 9, 9] -> [B, 6, 196, 9, 9]
        attn = attn * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = attn @ v                                                    # [B, 6, 196, 9, 9] @ [B, 6, 196, 9, 32] = [B, 6, 196, 9, 32]
        x = x.permute(0, 1, 4, 3, 2)                                    # [B, 6, 196, 9, 32] -> [B, 6, 32, 9, 196]
        x = x.reshape(B, C * self.kernel_size * self.kernel_size, h * w)# [B, 6, 32, 9, 196] -> [B, 6*32*9, 196]
        x = F.fold(x, output_size=(H, W), kernel_size=self.kernel_size, padding=self.padding, stride=self.stride)   # [B, 6*32*9, 196] -> [B, 192, 28, 28]

        x = x.permute(0, 2, 3, 1)   # [B, 192, 28, 28] -> [B, 28, 28, 192]
        x = self.proj(x)            # [B, 28, 28, 192] -> [B, 28, 28, 192]
        x = self.proj_drop(x)
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = qkv.reshape(B, H * W, 3, self.num_heads, C // self.num_heads) # [B, H, W, 3*C] -> [B, H*W, 3, h, c]  C = h * c
        qkv = qkv.permute(2, 0, 3, 1, 4)                                    # [B, H*W, 3, h, c] -> [3, B, h, H*W, c]
        q, k, v = qkv.unbind(0)                                             # [3, B, h, H*W, c] -> 3 * [B, h, H*W, c]

        attn = (q @ k.transpose(-2, -1)) * self.scale   # [B, h, H*W, c] @ [B, h, c, H*W] = [B, h, H*W, H*W]
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
...
class ClassAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        kv = self.kv(x)                                         # [B, 197, 384] -> [B, 197, 768]
        kv = kv.reshape(B, N, 2, self.num_heads, self.head_dim) # [B, 197, 768] -> [B, 197, 2, 12, 32]
        kv = kv.permute(2, 0, 3, 1, 4)                          # [B, 197, 2, 12, 32] -> [2, B, 12, 197, 32]
        k, v = kv.unbind(0)                                     # [2, B, 12, 197, 32] -> 2 * [B, 12, 197, 32]
        q = self.q(x[:, :1, :])                                 # [B, 197, 384] get [B, 1, 384] -> [B, 1, 384]
        q = q.reshape(B, self.num_heads, 1, self.head_dim)      # [B, 1, 384] -> [B, 12, 1, 32]

        attn = ((q * self.scale) @ k.transpose(-2, -1))         # [B, 12, 1, 32] @ [B, 12, 32, 197] = [B, 12, 1, 197]
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        cls_embed = attn @ v                                    # [B, 12, 1, 197] @ [B, 12, 197, 32] = [B, 12, 1, 32]
        cls_embed = cls_embed.transpose(1, 2)                   # [B, 12, 1, 32] -> [B, 1, 12, 32]
        cls_embed = cls_embed.reshape(B, 1, self.head_dim * self.num_heads) # [B, 1, 12, 32] -> [B, 1, 384]

        cls_embed = self.proj(cls_embed)                        # [B, 1, 384] -> [B, 1, 384]
...
class ClassBlock(nn.Module):
    ...
    def forward(self, x):
        cls_embed = x[:, :1]                      # [:1] 会保留维度 [0] 不会保留维度 [B, 197, 384] get [B, 1, 384]
        cls_embed = cls_embed + self.drop_path(self.attn(self.norm1(x)))        # [B, 1, 384] + [B, 1, 384] = [B, 1, 384]
        cls_embed = cls_embed + self.drop_path(self.mlp(self.norm2(cls_embed))) # [B, 1, 384] + [B, 1, 384] = [B, 1, 384]
        return torch.cat([cls_embed, x[:, 1:]], dim=1)                          # [B, 1, 384] cat [B, 196, 384] = [B, 197, 384]
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        if self.conv is not None:
            x = self.conv(x)    # [B, 3, 224, 224] -> [B, 64, 112, 112]
        x = self.proj(x)        # [B, 64, 112, 112] -> [B, 192, 28, 28]
        return x
...
class Downsample(nn.Module):
    ...
    def forward(self, x):
        x = x.permute(0, 3, 1, 2)   # [B, 28, 28, 192] -> [B, 192, 28, 28]
        x = self.proj(x)            # [B, 192, 28, 28] -> [B, 384, 14, 14]
        x = x.permute(0, 2, 3, 1)   # [B, 384, 14, 14] -> [B, 14, 14, 384]
        return x
...
class VOLO(nn.Module):
    ...
    def __init__(
        ...
        # outlook attention + self attention
        network = []
    ...
    def forward_tokens(self, x):
        ...
        for idx, block in enumerate(self.network):
            ...
            if self.grad_checkpointing and not torch.jit.is_scripting():
                ...
                x = block(x)    # [B, 28, 28, 192] -> [B, 14, 14, 384]

        B, H, W, C = x.shape
        x = x.reshape(B, -1, C) # [B, 14, 14, 384] -> [B, 196, 384]
        return x

    def forward_cls(self, x):
        """添加cls token,并使用ClassAttention"""
        B, N, C = x.shape   # [B, 196, 384]
        cls_tokens = self.cls_token.expand(B, -1, -1)   # [1, 1, 384] -> [B, 1, 384]
        x = torch.cat([cls_tokens, x], dim=1)           # [B, 196, 384] cat [B, 1, 384] = [B, 197, 384]
        for block in self.post_network:
            if self.grad_checkpointing and not torch.jit.is_scripting():
                x = checkpoint(block, x)
            else:
                x = block(x)    # [B, 197, 384] -> [B, 197, 384]
        return x
    ...
    def forward_features(self, x):
        x = self.patch_embed(x).permute(0, 2, 3, 1)  # [B, 3, 224, 224] -> [B, 192, 28, 28] -> [B, 28, 28, 192]

        # step2: tokens learning in the two stages
        x = self.forward_tokens(x)  # [B, 28, 28, 192] -> [B, 14*14, 384]

        # step3: post network, apply class attention or not
        if self.post_network is not None:
            x = self.forward_cls(x) # [B, 14*14, 384] -> [B, 14*14+1, 384]
        x = self.norm(x)
        return x
    ...
    def forward_head(self, x, pre_logits: bool = False):
        if self.global_pool == 'avg':
            ...
            out = x[:, 0]   # [B, 197, 384] get [B, 384]
        else:
            out = x
        x = self.head_drop(x)
        if pre_logits:
            return out
        out = self.head(out)# [B, 384] -> [B, num_classes]
        if self.aux_head is not None:
            # generate classes in all feature tokens, see token labeling
            aux = self.aux_head(x[:, 1:])   # [B, 197, 384] get [B, 196, 384] -> [B, 196, num_classes]
            out = out + 0.5 * aux.max(1)[0] # [B, num_classes] + 0.5 * ([B, 196, num_classes] -> [B, num_classes]) = [B, num_classes]
        return out

    def forward(self, x):
        """ simplified forward (without mix token training) """
        x = self.forward_features(x)    # [B, 3, 224, 224] -> [B, 197, 384]
...
if __name__ == "__main__":
    ...
    model = volo_d1_224(pretrained=False, num_classes=5).to(device)
