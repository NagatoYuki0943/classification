...
def conv2d(in_channel, out_channel, kernel_size, stride=1, groups=1):
    ...
    return nn.Sequential(
        nn.Conv2d(in_channel, out_channel, kernel_size=kernel_size, stride=stride, padding=pad, bias=False, groups=groups),
...
class FPNDown(nn.Module):
    def __init__(self, channels: list[int], steps: int = 5, groups: bool = False) -> None:
        """下采样FPN
        ...
        for i in range(len(channels)-1):
            # 调整通道数和宽高
            self.subs.append(nn.Sequential(conv2d(channels[i], channels[i+1], 1), nn.MaxPool2d(2)))
            # 调整后的数据和下一层相加后混合通道
            self.convs.append(nn.Sequential(conv2d(channels[i+1], channels[i+1], 3)))

        # 对最终的数据进行计算
        final_conv = []
        for i in range(steps):
            final_conv.append(conv2d(channels[-1], channels[-1], kernel_size=3 if i % 2 == 0 else 1, groups=channels[-1] if groups else 1))
...
class FPNUp(nn.Module):
    def __init__(self, channels: list[int], steps: int = 5, groups: bool = False) -> None:
        ...
        for i in range(len(channels)-1):
            # 调整通道数和宽高
            self.ups.append(nn.Sequential(conv2d(channels[i], channels[i+1], 1), nn.UpsamplingBilinear2d(scale_factor=2)))
            # 调整后的数据和下一层相加后混合通道
            self.convs.append(nn.Sequential(conv2d(channels[i+1], channels[i+1], 3)))
...
class FPNModel(nn.Module):
    def __init__(self, model_name: str = "resnet18",
        ...
        """
        super().__init__()
        self.model = timm.create_model(model_name, pretrained=pretrained, features_only=True, out_indices=out_indices)   # 一般选择后3层
