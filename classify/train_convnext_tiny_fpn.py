...
from modified_model.fpn import FPNModel
import os
...
NUM_CLASSES = 47
PRETRAINED  = True
SAVE_PATH   = f"./checkpoints/{time.strftime('%Y%m%d-%H%M%S', time.localtime())}-convnext_tiny_fpn"
...
if __name__ == "__main__":
    ...
    # dataset
    _, _, _, train_dataloader, val_dataloader, test_dataloader = get_torchvision_cls_dataset('DTD', BATCH_SIZE, resize=RESIZE, root="./datasets")

    # model
    model = FPNModel("convnext_tiny", out_indices=[1, 2, 3], num_classes=NUM_CLASSES, groups=True)
