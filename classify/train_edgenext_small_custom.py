...
NUM_CLASSES = 6
PRETRAINED  = True
SAVE_PATH   = f"./checkpoints/{time.strftime('%Y%m%d-%H%M%S', time.localtime())}-edgenext_small"
...
if __name__ == "__main__":
    ...
    # dataset
    _, _, train_dataloader, val_dataloader = get_custom_data("datasets/intel_origin_scenery", batch_size=BATCH_SIZE, resize=RESIZE, valid_resize=320)

    # model
    model = models.edgenext.edgenext_small(pretrained=True, num_classes=NUM_CLASSES)
