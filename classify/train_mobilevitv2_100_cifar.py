...
PRETRAINED = True
SAVE_PATH = (
    f"./checkpoints/{time.strftime('%Y%m%d-%H%M%S', time.localtime())}-mobilevitv2_100"
)
...
if __name__ == "__main__":
    ...
    # dataset
    _, _, train_dataloader, val_dataloader = cifar(
        variance="CIFAR10", batch_size=BATCH_SIZE, resize=RESIZE, root="./datasets"
    )

    # model
    model = models.mobilevit.mobilevitv2_100(pretrained=True, num_classes=NUM_CLASSES)
