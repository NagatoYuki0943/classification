...
from torchvision.transforms import v2   # https://pytorch.org/vision/stable/auto_examples/transforms/plot_transforms_getting_started.html
# 防止下载数据集报错
import ssl
...
def cifar(
    ...
    Returns:
        train_dataset, val_dataset, train_dataloader, val_dataloader
    """
    assert variance in ["CIFAR10", "CIFAR100"], "variance should in CIFAR10 and CIFAR100"
    dataset = {
...
def get_transforms(mode="Default", resize=224, valid_resize=224):
    ...
    train_transform = v2.Compose([
        ...
        v2.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.1), # 参数1为亮度，参数2为对比度，参数3为饱和度，参数4为色相
...
def get_torchvision_cls_dataset(
    ...
    """
    assert variance in ['DTD', 'FGVCAircraft', 'Flowers102', 'Food101', 'ImageNet', 'Places365', 'StanfordCars', 'SUN397'], \
        "variance should in DTD, FGVCAircraft, Flowers102, StanfordCars and ImageNet!"

    if valid_resize is None:
        valid_resize = resize
    train_transform, val_transform = get_transforms(transform_mode, resize, valid_resize)
    ...
    train_dataset    = dataset(root, split="train", transform=train_transform, download=download)
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=True, drop_last=False)

    val_dataset      = dataset(root, split="val", transform=val_transform, download=download)
    val_dataloader   = DataLoader(val_dataset, batch_size=batch_size, num_workers=num_workers, pin_memory=True, drop_last=False)

    test_dataset     = dataset(root, split="test", transform=val_transform, download=download)
    test_dataloader  = DataLoader(test_dataset, batch_size=batch_size, num_workers=num_workers, pin_memory=True, drop_last=False)

    train_length = len(train_dataset)
    val_length   = len(val_dataset)
    test_length  = len(test_dataset)
    print(f"\033[0;32;40mtrain images: {train_length}, val images: {val_length}, test images: {test_length}\033[0m")
...
if __name__ == "__main__":
    _, _, train_dataloader, _ = cifar("CIFAR10", root="../datasets", batch_size=24, resize=32)
    image_target = next(iter(train_dataloader))
    print(image_target[0].shape)    # [24, 3, 32, 32]
    print(image_target[1])          # [7, 2, 6, 8, 9, 9, 6, 0, 3, 4, 2, 1, 1, 3, 6, 8, 2, 2, 8, 3, 0, 1, 7, 2]
