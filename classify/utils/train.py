...
def train_epochs(
    ...
    batch_size: int,
    num_workers=0,
    save_path="./checkpointss/",
    save_interval=10,
    lr_sche: LRScheduler = None,
    half=False,
    distributed=False,
    sync_bn=False,
):
    ...
    for epoch in range(1, epochs + 1):  # 1~epochs
        ...
        if local_rank == 0:
            ...
            pbar = tqdm(
                total=len(train_dataloader), desc=f"Epoch {epoch}/{epochs} train"
            )
        # ----------------------------------------#
        #   train
        # ----------------------------------------#
        ...
        for image, target in train_dataloader:
            ...
            if not cuda and not half:
                ...
                scaler.scale(
                    loss
                ).backward()  # 用scaler，scale loss(FP16)，backward得到scaled的梯度(FP16)
                scaler.step(
                    optimizer
        ...
        for image, target in val_dataloader:
            ...
            acc_arr.append(
                acc
            )  # 这里如果 +=pred.eq(target).float().sum().item() 准确率会降低到50%，不知道怎么计算的
        ...
        if local_rank == 0:
            ...
            if epoch % save_interval == 0:
                torch.save(
                    model.state_dict(),
                    f"{save_path}/{str(epoch)}_{str(val_avg_acc)}_model.pth",
                )
                torch.save(
                    optimizer.state_dict(),
                    f"{save_path}/{str(epoch)}_{str(val_avg_acc)}_optim.pth",
                )
