...
from pytorch_lightning.callbacks import LearningRateMonitor, DeviceStatsMonitor, ModelCheckpoint, EarlyStopping
...
class LightningTrainer(pl.LightningModule):
    ...
    def epoch_end(self, step_outputs):
        ...
        acc_avg  = accuracy(preds=preds, target=targets, task="multiclass", num_classes=self.num_classes, top_k=1)
        p        = precision(preds=preds, target=targets, task="multiclass", num_classes=self.num_classes, average=None)
        r        = recall(preds=preds, target=targets, task="multiclass", num_classes=self.num_classes, average=None)
        cm       = confusion_matrix(preds=preds, target=targets, task="multiclass", num_classes=self.num_classes, normalize="true")
    ...
    def training_step(self, batch, *args, **kwargs):
        ...
        acc = accuracy(preds=preds, target=targets, task="multiclass", num_classes=self.num_classes, top_k=1)
    ...
    def on_test_epoch_end(self):
        """在一个测试epoch结尾处被调用
        """
        loss_avg, acc_avg, precision, recall, confusion_matrix = self.epoch_end(self.test_step_outputs)
...
def lightning_trainer(
    ...
    """
    assert num_classes > 0, "\033[0;31;40mnum_classes must be an integer greater than 0 !\033[0m"

    print(f"\033[0;34;40m==================== training files save to {save_path} ====================\033[0m")
    ...
    # loggder
    loggder = [CSVLogger(save_dir = save_path, name = "csv_logs", version = "")]  # name="" 保存路径就为 save_dir/version
    tensorboard_logger = TensorBoardLogger(save_dir = save_path, name = "tensorboard_logs", version = "")
    tensorboard_logger.log_hyperparams(hyperparams)
    loggder.append(tensorboard_logger)
    print(f"\033[0;32;40m====== tensorboard log success, please use `tensorboard --logdir=tensorboard_logs` in {save_path} to check metrics ======\033[0m")

    # wandb https://wandb.ai/site
    try:
        wandb_loggder = WandbLogger(save_dir = save_path,
                                    project  = project if project is not None else train_dataloader.dataset.__class__.__name__)
        wandb_loggder.log_hyperparams(hyperparams)
        loggder.append(wandb_loggder)
    except:
        print("\033[0;33;40m================== wandb log fail, please use `pip install wandb` to install wandb(optional) ==================\033[0m")
