...
#   记录数据
#-------------------------------------#
def record(epoch, lr, train_loss, val_loss, val_acc, best_acc, times,
           time_now=time.strftime("%Y%m%d-%H%M%S",time.localtime()), file="checkpoint/records.csv"):
    """记录数据
    ...
    with open(file, mode='a', encoding='utf-8') as f:
        s = str(epoch) + ',' + str(lr) + ',' + str(train_loss) + "," + str(val_loss) + "," + \
            str(val_acc) + "," + str(best_acc) + "," + str(times) + ',' + time_now + "\n"
        if epoch==1:
...
def test_record():
    for i in range(100):
        time.sleep(0.1)
        print(i)
        record(i+1, random.random(), random.random(), random.random(), random.random(), random.random(),
               random.randint(100, 200), time.strftime("%Y%m%d-%H%M%S", time.localtime()), "../checkpoints/records.csv")
...
def test_show_config():
    ...
    lr_sche         = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=100, eta_min=0.001*0.01)    # 最小减小100倍
    ...
    show_config(epochs=epochs, save_interval=save_interval, device=device,
                model=model.__class__.__name__, optimizer=optimizer.__class__.__name__ + ": " + " ".join(optim_param),
                lr_sche=lr_sche.__class__.__name__ + ": " + " ".join(lr_param) if lr_sche else None,
                loss_fn=loss_fn.__class__.__name__, batch_size=batch_size,\
                half=half, distributed=distributed, sync_bn=sync_bn, save_path=save_path)
    # Configurations:
    # ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    # |                keys |                                                                                                                                                      values|
    # ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
def get_optimizer(name: str, model, lr: float, **kwargs):
    ...
    """
    name = name.lower()
    assert name in ['sgd', 'adam', 'adamw', 'adagrad', 'adadelta', 'rmsprop'], "optimizer name error"
    print(name)
    if name =='sgd':
