...
class ConfusionMatrix(object):
    ...
    def summary(self):
        ...
        for i in range(self.num_classes):
            ...
            TN = np.sum(self.matrix) - TP - FP - FN # 总和 - TP - FP - FN = TN 判断正确的N
            Precision   = round(TP / (TP + FP), 3) if TP + FP != 0 else 0.      # 精确率,查的准不准 round(..., 3)  保留3位小数
            Recall      = round(TP / (TP + FN), 3) if TP + FN != 0 else 0.      # 召回率,查的全不全
            Specificity = round(TN / (TN + FP), 3) if TN + FP != 0 else 0.      # 特异度,Negative的召回率
            table.add_row([self.labels[i], Precision, Recall, Specificity])     # 添加到table中 参数1是标签
        print(table)
        ...
        for x in range(self.num_classes):
            for y in range(self.num_classes):
                ...
                plt.text(x, y, info,
                         ...
                         color="white" if info > thresh else "black")   # 大于阈值就是白色文字,否则是黑色
